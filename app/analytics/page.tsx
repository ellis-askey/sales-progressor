...
import { getAnalytics, getReferralStats, getBrokerReferralStats } from "@/lib/services/analytics";
...
export default async function AnalyticsPage() {
  ...
  const [data, referralStats, brokerReferralStats, taskCounts, todoCount] = await Promise.all([
    getAnalytics(session.user.agencyId),
    getReferralStats(session.user.agencyId),
    getBrokerReferralStats(session.user.agencyId),
      ...
      <div className="px-8 py-7 space-y-6">
        ...
        {referralStats.length > 0 && (
          <div className="glass-card">
            <div className="px-5 py-4 border-b border-white/20 flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-slate-900/90">Referral income · Conveyancers</p>
        ...
        {brokerReferralStats.length > 0 && (
          <div className="glass-card">
            <div className="px-5 py-4 border-b border-white/20 flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-slate-900/90">Referral income · Brokers</p>
                <p className="text-xs text-slate-900/40 mt-0.5">Mortgage broker referral fee tracking</p>
              </div>
              <div className="text-right">
                <p className="text-lg font-bold text-emerald-600">
                  {fmt(brokerReferralStats.reduce((s, r) => s + r.feeReceivedPence, 0))}
                </p>
                <p className="text-xs text-slate-900/40">received</p>
              </div>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-slate-900/40 uppercase tracking-wide border-b border-white/10">
                  <th className="px-5 py-3 text-left font-medium">Broker firm</th>
                  <th className="px-5 py-3 text-right font-medium">Referrals</th>
                  <th className="px-5 py-3 text-right font-medium">Expected</th>
                  <th className="px-5 py-3 text-right font-medium">Received</th>
                  <th className="px-5 py-3 text-right font-medium">Pending</th>
