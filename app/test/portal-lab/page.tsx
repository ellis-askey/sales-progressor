...
//
// Every element flagged in the 2026-08-15 portal revamp brief, rendered on the
// real portal light background with live options. Pick by copying the ID under
// each option (e.g. "PRIMARY-A3, COMMS-C2, PILL-P2, SURVEY-S2, LOADING-L1,
// DATES-D2") back to me and I wire the winners into the live portal.
//
// Not shipped product - no prod block needed; the /test layout already 404s
// for everyone but the founder. Delete this route once the picks are locked.
// ─────────────────────────────────────────────────────────────────────────────

import { useState } from "react";
import { P } from "@/components/portal/portal-ui";

// Injected keyframes + interaction helpers. Kept inline so the lab is fully
// self-contained and leaves no trace in globals.css when we delete it.
const LAB_CSS = `
...
// Apple system palette used across the button + pill options.
const APPLE = {
  blue:      "#0A84FF",
  blueDeep:  "#0060DF",
  blueTint:  "rgba(10,132,255,0.12)",
  green:     "#25D366",
  greenDeep: "#1FAE53",
  greenTint: "rgba(37,211,102,0.14)",
  ink:       "#1A1D29",
};

// ── Small building blocks ────────────────────────────────────────────────────

function OptionCard({ id, note, children }: { id: string; note?: string; children: React.ReactNode }) {
  return (
    <div style={{ background: P.cardBg, borderRadius: 16, boxShadow: P.shadowSm, padding: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 12 }}>
        <code style={{
          ...
          color: P.accent, background: P.accentBg, padding: "3px 8px", borderRadius: 6,
        }}>{id}</code>
...
function Section({ letter, title, blurb, children }: { letter: string; title: string; blurb: string; children: React.ReactNode }) {
  return (
    <section id={`sec-${letter}`} style={{ scrollMarginTop: 64 }}>
...
const PRIMARY_OPTIONS = [
  {
    id: "PRIMARY-A1", note: "Current - flat coral, big glow",
    render: () => (
      <button className="lab-press" style={{ ...fullBtnBase(), borderRadius: 16, background: P.primary, boxShadow: P.heroGlow }}>
        Confirm this step
      </button>
    ),
  },
  {
    id: "PRIMARY-A2", note: "Refined coral - soft gradient, top sheen, gentle press",
    render: () => (
      <button className="lab-press" style={{
        ...fullBtnBase(),
        background: "linear-gradient(180deg,#FF7A5A 0%,#FF5A38 100%)",
        boxShadow: "0 1px 2px rgba(204,74,46,0.40), 0 8px 20px rgba(255,107,74,0.26), inset 0 1px 0 rgba(255,255,255,0.28)",
      }}>Confirm this step</button>
    ),
  },
  {
    id: "PRIMARY-A3", note: "Tactile coral - deeper press (scale + drop), physical feel",
    render: () => (
      <button className="lab-press-deep lab-press" style={{
        ...fullBtnBase(),
        background: "linear-gradient(180deg,#FF6F4E 0%,#F04E2C 100%)",
        boxShadow: "0 2px 0 #C63E20, 0 6px 16px rgba(240,78,44,0.30), inset 0 1px 0 rgba(255,255,255,0.30)",
      }}>Confirm this step</button>
    ),
  },
  {
    id: "PRIMARY-A4", note: "Ink premium - non-coral, calm and expensive",
    render: () => (
      <button className="lab-press" style={{
        ...fullBtnBase(),
        background: "linear-gradient(180deg,#2A2E3C 0%,#14161F 100%)",
        boxShadow: "0 1px 2px rgba(0,0,0,0.30), 0 8px 20px rgba(15,23,42,0.22), inset 0 1px 0 rgba(255,255,255,0.10)",
      }}>Confirm this step</button>
    ),
  },
];
...
const COMMS_OPTIONS = [
  {
    id: "COMMS-C1", note: "Current - green solid + coral-tinted email",
    render: () => (
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <CommsBtn className="lab-press" bg="#25D366" fg="#fff" icon="wa">WhatsApp</CommsBtn>
        <CommsBtn className="lab-press" bg={P.primaryBg} fg={P.primaryText} icon="mail">Email</CommsBtn>
      </div>
    ),
  },
  {
    id: "COMMS-C2", note: "Solid pair - Apple green + Apple blue, depth + press",
    render: () => (
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <CommsBtn className="lab-press" bg="linear-gradient(180deg,#2ED974 0%,#1FAE53 100%)" fg="#fff" icon="wa"
          shadow="0 1px 2px rgba(31,174,83,0.4), 0 6px 14px rgba(37,211,102,0.24), inset 0 1px 0 rgba(255,255,255,0.3)">WhatsApp</CommsBtn>
        <CommsBtn className="lab-press" bg="linear-gradient(180deg,#2A93FF 0%,#0A6FE8 100%)" fg="#fff" icon="mail"
          shadow="0 1px 2px rgba(0,96,223,0.4), 0 6px 14px rgba(10,132,255,0.24), inset 0 1px 0 rgba(255,255,255,0.3)">Email</CommsBtn>
      </div>
    ),
  },
  {
    id: "COMMS-C3", note: "Tonal pair - soft tinted bg, coloured text (quiet, modern)",
    render: () => (
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <CommsBtn className="lab-press" bg={APPLE.greenTint} fg={APPLE.greenDeep} icon="wa">WhatsApp</CommsBtn>
        <CommsBtn className="lab-press" bg={APPLE.blueTint} fg={APPLE.blueDeep} icon="mail">Email</CommsBtn>
      </div>
    ),
  },
  {
    id: "COMMS-C4", note: "Mixed - WhatsApp keeps its brand green, Email goes blue tonal",
    render: () => (
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <CommsBtn className="lab-press" bg="#25D366" fg="#fff" icon="wa"
          shadow="0 4px 12px rgba(37,211,102,0.24), inset 0 1px 0 rgba(255,255,255,0.28)">WhatsApp</CommsBtn>
        <CommsBtn className="lab-press" bg={APPLE.blueTint} fg={APPLE.blueDeep} icon="mail">Email</CommsBtn>
      </div>
    ),
  },
...
function CommsBtn({ children, bg, fg, icon, shadow, className }: {
  children: React.ReactNode; bg: string; fg: string; icon: "wa" | "mail"; shadow?: string; className?: string;
}) {
  return (
    <span className={className} style={{
      display: "inline-flex", alignItems: "center", gap: 7, cursor: "pointer",
      fontSize: 13, fontWeight: 700, padding: "9px 14px", borderRadius: 11,
...
function MailIcon() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden style={{ flexShrink: 0 }}><path d="M20 4H4a2 2 0 00-2 2v12a2 2 0 002 2h16a2 2 0 002-2V6a2 2 0 00-2-2m0 4l-8 5-8-5V6l8 5 8-5z" /></svg>;
}
...
const PILL_OPTIONS = [
  {
    id: "PILL-P1", note: "Current - tinted bg, coloured text, no dot",
    render: (t: PillTone) => (
      <span style={{ fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 999, background: t.bg, color: t.fg }}>{t.label}</span>
    ),
  },
  {
    id: "PILL-P2", note: "Dotted tonal - status dot + text (Apple status idiom)",
    render: (t: PillTone) => (
  ...
  {
    id: "PILL-P3", note: "Hairline glass - white bg, 0.5px coloured border, dot",
    render: (t: PillTone) => (
  ...
  {
    id: "PILL-P4", note: "Solid micro - saturated bg, white text (loud, high-contrast)",
    render: (t: PillTone) => (
...
// ── Survey quote card previews ───────────────────────────────────────────────

function ChevRight({ color = P.accent }: { color?: string }) {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><polyline points="9 18 15 12 9 6" /></svg>;
}
function MagnifierHouse({ size = 22, color = P.accent }: { size?: number; color?: string }) {
...
const SURVEY_OPTIONS = [
  {
    id: "SURVEY-S1", note: "Current - icon tile + text + chevron",
    render: () => (
      <div className="lab-press" style={{ display: "flex", alignItems: "center", gap: 12, padding: 16, borderRadius: 16, background: P.cardBg, boxShadow: P.shadowSm, cursor: "pointer" }}>
  ...
  {
    id: "SURVEY-S2", note: "Imagery banner - gradient header motif, price hint chip",
    render: () => (
      <div className="lab-press" style={{ borderRadius: 16, overflow: "hidden", background: P.cardBg, boxShadow: P.shadowMd, cursor: "pointer" }}>
  ...
  {
    id: "SURVEY-S3", note: "Trust chips + CTA button - benefits made explicit",
    render: () => (
  ...
  {
    id: "SURVEY-S4", note: "Standout accent - full blue-tinted card, big price",
    render: () => (
      <div className="lab-press" style={{ borderRadius: 16, cursor: "pointer", padding: 16, background: "linear-gradient(160deg, rgba(10,132,255,0.10), rgba(10,132,255,0.03))", border: `0.5px solid ${APPLE.blueTint}`, boxShadow: P.shadowSm, display: "flex", alignItems: "center", gap: 14 }}>
...
export default function PortalLabPage() {
  ...
  const [loadMode, setLoadMode] = useState<"L1" | "L2">("L1");
  const [helpFixed, setHelpFixed] = useState(true);

  const NAV = [
    ["A", "Buttons"], ["B", "Pills"], ["C", "Survey card"], ["D", "Micro-interactions"],
    ["E", "Loading"], ["F", "Help button"], ["G", "Important dates"],
  ];

  return (
    <div style={{ minHeight: "100vh", background: P.pageBg, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      <style dangerouslySetInnerHTML={{ __html: LAB_CSS }} />

      {/* Sticky header + jump nav */}
      <div style={{ position: "sticky", top: 0, zIndex: 20, background: "rgba(248,249,251,0.9)", backdropFilter: "blur(12px)", borderBottom: `1px solid ${P.border}` }}>
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "12px 16px 10px" }}>
          ...
          <p style={{ margin: 0, fontSize: 12, color: P.textSecondary }}>Copy the ID under any option you like and send it back. e.g. <code style={{ fontSize: 11, color: P.textPrimary }}>PRIMARY-A3, COMMS-C2, PILL-P2, SURVEY-S2</code></p>
      ...
      <main style={{ maxWidth: 640, margin: "0 auto", padding: "24px 16px 80px", display: "flex", flexDirection: "column", gap: 40 }}>

        {/* ── A · Buttons ─────────────────────────────────────────── */}
        <Section letter="A" title="Buttons" blurb="Apple-grade overhaul of the primary CTA (Confirm this step / Add) and the contact pair (WhatsApp + Email). Tap any button to feel the press. Email options are all blue, per your call.">
          <p style={{ margin: 0, fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: P.textMuted }}>Primary CTA</p>
          {PRIMARY_OPTIONS.map((o) => <OptionCard key={o.id} id={o.id} note={o.note}>{o.render()}</OptionCard>)}
          <p style={{ margin: "8px 0 0", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: P.textMuted }}>Small primary (&ldquo;Add&rdquo;)</p>
          <OptionCard id="PRIMARY-SM" note="The winning primary style, shrunk to the team-card Add size (I match whatever you pick above)">
          ...
          {COMMS_OPTIONS.map((o) => <OptionCard key={o.id} id={o.id} note={o.note}>{o.render()}</OptionCard>)}
        </Section>

        {/* ── B · Pills ───────────────────────────────────────────── */}
        <Section letter="B" title="Pills" blurb="The who/status pills used in Coming up (You / Your lender / Solicitor), the hero (Active / Freehold / Mortgage) and the timeline (Completed / In progress / Pending). Same six labels rendered four ways.">
          {PILL_OPTIONS.map((o) => (
            <OptionCard key={o.id} id={o.id} note={o.note}>
        ...
        {/* ── C · Survey card ─────────────────────────────────────── */}
        <Section letter="C" title="Survey quote card" blurb="The &ldquo;Get a survey quote&rdquo; prompt buyers see once instructed. Options add imagery, a price anchor, and trust cues so it stands out instead of reading like a plain row.">
          {SURVEY_OPTIONS.map((o) => <OptionCard key={o.id} id={o.id} note={o.note}>{o.render()}</OptionCard>)}
        </Section>

        {/* ── D · Micro-interactions ──────────────────────────────── */}
        <Section letter="D" title="Micro-interactions" blurb="The two motion touches you asked for. Hover (desktop) or tap (mobile) to see them.">
          <OptionCard id="MICRO-CHEV-EASE" note="View full timeline - chevron glides right, smooth ease">
            <span className="lab-chev" style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "6px 12px", borderRadius: 999, fontSize: 12, fontWeight: 600, color: P.accent, background: P.accentBg, cursor: "pointer" }}>
              View full timeline <span className="lab-chev-i" style={{ display: "inline-flex" }}><ChevRight /></span>
            </span>
          </OptionCard>
          <OptionCard id="MICRO-CHEV-SPRING" note="Same, with a springier overshoot on the chevron">
            <span className="lab-chev lab-chev-spring" style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "6px 12px", borderRadius: 999, fontSize: 12, fontWeight: 600, color: P.accent, background: P.accentBg, cursor: "pointer" }}>
              View full timeline <span className="lab-chev-i" style={{ display: "inline-flex" }}><ChevRight /></span>
            </span>
          </OptionCard>
          <OptionCard id="MICRO-PRESS" note="Press-down on the CTA - the same feel ships on whichever primary you pick in A">
        ...
        {/* ── E · Loading ─────────────────────────────────────────── */}
        <Section letter="E" title="Loading treatment" blurb="Replaces the grey skeletons. The screen edge pulses blue while the content settles, then everything fades up into place. Hit Replay to watch it again.">
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <button onClick={() => { setLoadMode("L1"); setLoadKey((k) => k + 1); }} style={{ cursor: "pointer", fontSize: 12, fontWeight: 600, padding: "8px 12px", borderRadius: 10, border: `1px solid ${loadMode === "L1" ? P.accent : P.border}`, background: loadMode === "L1" ? P.accentBg : "#fff", color: loadMode === "L1" ? P.accent : P.textSecondary }}>LOADING-L1 · fade together</button>
            <button onClick={() => { setLoadMode("L2"); setLoadKey((k) => k + 1); }} style={{ cursor: "pointer", fontSize: 12, fontWeight: 600, padding: "8px 12px", borderRadius: 10, border: `1px solid ${loadMode === "L2" ? P.accent : P.border}`, background: loadMode === "L2" ? P.accentBg : "#fff", color: loadMode === "L2" ? P.accent : P.textSecondary }}>LOADING-L2 · cascade</button>
          </div>
          <div style={{ position: "relative", borderRadius: 20, overflow: "hidden", background: P.pageBg, boxShadow: P.shadowSm, padding: 14, minHeight: 320 }}>
            {/* Edge pulse - plays for the first ~1.4s on each replay */}
          ...
          <p style={{ margin: 0, fontSize: 12, color: P.textMuted, lineHeight: 1.5 }}>My pick: <b style={{ color: P.textPrimary }}>L1</b> - everything settles as one calm motion, no flicker. L2 cascades card by card if you want more theatre.</p>
        </Section>

        {/* ── F · Help button ─────────────────────────────────────── */}
        <Section letter="F" title="Help button position" blurb="The floating Help pill currently rests 24px off the bottom, so it collides with the portal's bottom nav. Toggle to see the fix: it lifts to sit clear above the bar.">
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => setHelpFixed(false)} style={{ cursor: "pointer", fontSize: 12, fontWeight: 600, padding: "8px 12px", borderRadius: 10, border: `1px solid ${!helpFixed ? P.primary : P.border}`, background: !helpFixed ? P.primaryBg : "#fff", color: !helpFixed ? P.primary : P.textSecondary }}>Current (overlaps)</button>
            <button onClick={() => setHelpFixed(true)} style={{ cursor: "pointer", fontSize: 12, fontWeight: 600, padding: "8px 12px", borderRadius: 10, border: `1px solid ${helpFixed ? P.accent : P.border}`, background: helpFixed ? P.accentBg : "#fff", color: helpFixed ? P.accent : P.textSecondary }}>HELP-FIX (raised)</button>
          </div>
          ...
          <p style={{ margin: 0, fontSize: 12, color: P.textMuted, lineHeight: 1.5 }}>The fix is portal-scoped, so the agent app (no bottom nav) keeps the Help pill where it is today.</p>
        </Section>

        {/* ── G · Important dates ─────────────────────────────────── */}
        <Section letter="G" title="Important dates" blurb="How it works today: a row appears only for a step that is confirmed done AND had a specific date attached at confirmation (instruct solicitor, submit mortgage application, exchange, completion). It is never predicted. Two ways to present the same data:">
          <OptionCard id="DATES-D1" note="Current - label left, date right, plain list">
          ...
          <OptionCard id="DATES-D2" note="Timeline - calendar chip + relative time, warmer and scannable">
            <div style={{ borderRadius: 16, overflow: "hidden", background: P.cardBg, boxShadow: P.shadowSm }}>
              <div style={{ padding: "12px 16px", borderBottom: `1px solid ${P.border}` }}><p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: P.textPrimary }}>Important dates</p></div>
