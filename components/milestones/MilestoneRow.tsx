...
export function MilestoneRow({ def, transactionId, onRefresh }: Props) {
  ...
  if (!isDone && isBlocked) rowBg = "bg-gray-50/40";
  ...
  return (
    <>
      <div className={`flex items-start gap-3 pl-4 pr-5 py-3.5 border-b border-[#f0f4f8] last:border-0 ${rowBg}`}>
        {/* Timeline node */}
        ...
        <div className="flex-1 min-w-0">
          ...
          {isBlocked && <p className="text-xs text-gray-400 mt-0.5">Previous milestones must be completed first</p>}
          ...
          {showNotRequired && !isPM4 && (
            <div className="mt-2 flex items-start gap-2">
              ...
              <button onClick={() => { setShowNotRequired(false); setNotRequiredReason(""); }} className="mt-5 text-xs text-gray-400 hover:text-gray-600">Cancel</button>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          ...
          {isDone && (
            <button onClick={handleReverseClick} disabled={loading} className="text-xs text-gray-300 hover:text-red-400 transition-colors">
              Undo
            </button>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              <p className="text-sm font-semibold text-gray-800">How is the buyer purchasing?</p>
      ...
      {showImpliedModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4">
          <div className="bg-white rounded-2xl border border-[#e4e9f0] shadow-xl max-w-md w-full p-6" style={{ boxShadow: "0 8px 32px rgba(0,0,0,0.12)" }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.35)" }}>
          <div className="bg-white rounded-2xl border border-[#e4e9f0] shadow-xl max-w-md w-full p-6" style={{ boxShadow: "0 8px 32px rgba(0,0,0,0.12)" }}>
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <h3 className="text-base font-semibold text-gray-900">This will undo other milestones</h3>
                <p className="text-sm text-gray-500 mt-0.5">Undoing <strong>"{def.name}"</strong> means the following completed milestones can no longer be true:</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 bg-gray-50 rounded-xl px-4 py-3 mb-5">
              <div className="text-center">
                <p className="text-xs text-gray-400 mb-0.5">Current</p>
                <p className="text-xl font-semibold text-gray-700">{currentPercent}%</p>
              </div>
