...
| Surface | Opportunity | Decision | Filed | Notes |
|---|---|---|---|---|
| `OutboundMessage.contactIds String[]` orphan risk on contact delete | Contact deletion cascades cleanly for `ClientChaseState`, `PortalMessage`, `PortalPushSubscription`; sets null on `TransactionDocument.contactId`. The one gap is `OutboundMessage.contactIds` — a plain `String[]` with no FK. Deleted-contact IDs stay in the array. No active read path dereferences them today (verified 2026-07-13), so it isn't a live bug, but the pattern will bite the next feature that resolves IDs against a `Map`. Options: (a) migrate to a proper join table, (b) sweep on delete via a `deleteContact` post-hook. | tbd | 2026-07-13 | Surfaced by the coherence-arc chunk 9 verification. `ChaseTask` itself is transaction-scoped and safe on contact delete. |
...
| [components/reminders/RemindersSection.tsx](../components/reminders/RemindersSection.tsx) group / Snoozed / Completed accordion headers | Migrate to canonical `Accordion.Header` | grandfather | 2026-06-30 | Same primitive gap as MilestonePanel: two-zone headers (left label+badge, right Show/Hide link) and the toggle lives on an explicit inner button, not the whole header. `Accordion.Header` would force the whole header to be the toggle (UX change) AND collapse the two zones into a single flex-gap row (layout regression). Surfaced during Wave A4. Re-evaluated 2026-09-26 |
| [components/reminders/AutomatedEmailsCard.tsx](../components/reminders/AutomatedEmailsCard.tsx) accordion header | Migrate to canonical `Accordion.Header` | grandfather | 2026-06-30 | Two-zone header (left icon+label, right Show/Hide button with custom CaretDown). Same Accordion.Header layout collision as above; additionally the existing built-in chevron from `Accordion.Header showChevron={true}` would conflict with the custom CaretDown inside the Show/Hide button. Surfaced during Wave A4. Re-evaluated 2026-09-26 |

### Phase 3 Surface 2 grandfathers (hub remediation)

| Surface | Opportunity | Decision | Filed | Notes |
|---|---|---|---|---|
| [app/agent/hub/page.tsx](../app/agent/hub/page.tsx) + [loading.tsx](../app/agent/hub/loading.tsx) `agent-glass` / `agent-glass-strong` / `agent-glass-light` chrome (15 instances) | Migrate to canonical `<Card variant="...">` | grandfather | 2026-06-30 | The hub's `agent-glass*` utilities are visually distinct from the Card primitive's `glass-card` chrome: different blur (24px vs 40px), different background tokens, different shadow, different radius (14px vs 12px). Card primitive `variant="glass"` does NOT map. Per [PLAN.md Option B](phase-3/02-agent-hub/PLAN.md), the hub keeps `agent-glass*` until the Card primitive grows a `variant="agent-glass"` extension (or until the agent-glass system gains a second consumer worth a primitive). Hub is the daily landing page so visible chrome shifts are high-stakes. Surfaced during Surface 2 audit. Re-evaluated 2026-09-26 |
...
### Phase 3 Surface 6 grandfathers (completions)

| Surface | Opportunity | Decision | Filed | Notes |
|---|---|---|---|---|
| Surface 6 `agent-glass` / `agent-glass-strong` cards (4 instances: `app/agent/completions/page.tsx:132` empty card + L154 + L183 ghost groups, `CompletionsGroupList.tsx:48` active group cards) | Migrate to `<Card variant="agent-glass">` | grandfather | 2026-06-30 | Surface 6 is now the **fifth consumer** of `agent-glass*` (hub + work-queue + transactions list + to-do + completions = 5 surfaces). Law 14 trigger is overwhelmingly active. Card primitive `variant="agent-glass"` extension is the next primitive batch's top priority. Re-evaluated 2026-09-26 |
