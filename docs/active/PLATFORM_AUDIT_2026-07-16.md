...
**Notes & decisions.** Grew into a 3-phase portal feature (mockups approved by Ellis: team card + a revamped settings drawer). Decisions locked with Ellis: contact details ARE shown for the progressor (WhatsApp + email); solicitor is firm-name only; sellers see no other agent; buyers who are also selling can add their own selling agent from the drawer; a client editing an agent does NOT auto-invite (the managing agent gets a bell notification and decides) — the first of "more than just confirmations" in the bell; a chain link goes read-only once the other agent has claimed it; client can add their OWN profile photo which feeds everywhere their face shows (bell, Updates feed, notes, portal). **PHASE 1 BUILT 2026-08-13 (the team card), awaiting push + staging test.** `getPortalTeam(txId, side)` in `lib/services/portal.ts` resolves the managing person (assignedUser for outsourced / agentUser for self-managed) with name, photo, the agency-resolved sender email (`resolveSenderForTransaction`), and a WhatsApp link (currently the single progressor number, outsourced-only); plus the viewer's own-side solicitor firm name. Rendered by `components/portal/PortalTeamCard.tsx` on the portal overview. tsc clean. **PHASE 2 BUILT 2026-08-13 (client photo + drawer header), awaiting migration + push + staging test.** Added `Contact.image` (migration `20260815120000_contact_image`, nullable, NOT yet applied — apply STAGING-FIRST at push time, before the Phase 2 code runs, then prod). Token-scoped upload route `app/api/portal/[token]/upload-avatar/route.ts` writes to the `avatars` bucket at `contact-{id}.{ext}` and stamps `Contact.image` (mirrors the agent avatar route). The portal menu drawer gained a profile header with the photo + a camera button to change it (`components/portal/PortalMenuDrawer.tsx`); `getMyPortalDetailsAction` now returns the image + address. The photo then feeds the agent-side "who did it" surfaces: the notification bell (`AgentBell` + notifications route) and the Updates feed (`CommsActivityFeed` + comms page), via `id`+`image` added to the contact select in `getAgentMilestoneActivity`, using the exact confirming contact (`confirmedByContactId`) when known. Falls back to initials / the generic silhouette. tsc clean, 188 tests pass. Minor deferred: the portal's OWN latest-updates feed still shows a green tick (not the client's face) on their own steps — low value, left for later. **PHASE 3 BUILT 2026-08-13 (the "Your agents" section) — #16 now DONE, awaiting push + staging test.** `getPortalChainAgent(txId, side)` reads the client's neighbouring chain link (seller → the link ABOVE = onward purchase; buyer → the link BELOW = their selling agent), pre-filled from the managing agent's stub. `updateMyChainAgentAction` (in `portal-menu.ts`) edits the stub / adds a link / creates the chain if none, attributing every chain write to the managing user (the client is a Contact, not a User) — and sends NO invite: it writes an Updates-feed note AND a `portal_chain_agent_updated` Notification. A link that's already been claimed (transactionId set) is read-only. The drawer gained a "Your agents" section (`YourAgentsSection` + `AgentEditForm`); the team card shows the buyer's selling-agent row with an Add/Edit button (`OpenAgentsButton`) that fires `portal:open-menu` → PortalShell opens the drawer and scrolls to the section. The bell now surfaces this event via a strict type allowlist (`BELL_NOTIFICATION_TYPES`) in the notifications route — the first non-confirmation notification, scoped to the caller's userId. tsc clean, 188 tests pass. Risks: chain writes from the portal — the topology (seller=above/buyer=below) and claimed-link read-only guard are the load-bearing bits; verify on staging before trusting on prod.
