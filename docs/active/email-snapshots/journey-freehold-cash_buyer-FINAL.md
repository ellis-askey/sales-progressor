# Journey: Freehold × Cash buyer

Final rewritten email matrix for this journey shape. All phases, every milestone, every variant.

**Voice:** Agency white-labelled "we". Transactional notification style. "Hi [name]" opener, no sign-off, ends at portal link.
**Lexicon:** No em dashes in body copy. No hedge crutches.
**Shape:** Freehold (no TA7, no management pack, Phase 7 suppressed). Cash buyer (mortgage suppressed, no related sale / no chain dependency, deposit transfer fires at PM24). See `SHAPE-NOTES.md`.

---

## Phase 1 — Instruction

### VM1 — Seller has instructed their solicitor

**Vendor — to the seller:**

**Subject:** Your solicitor's instructed, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor's instructed. That's conveyancing formally underway on your side.

They'll start preparing the contract pack now: title documents, the property forms you'll need to fill in (TA6 and TA10), and anything else that needs to go to the buyer's solicitor. The next thing you'll see from them is their welcome pack landing in the next few days.

When the welcome pack arrives, turning it around quickly is the single biggest thing you can do to keep the sale moving early on.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** The seller's solicitor is instructed, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller's solicitor is instructed. Conveyancing is now underway on their side.

They'll start preparing the contract pack: title documents and the property information forms. Once it's assembled, it goes across to your solicitor to review.

Nothing for you to do right now. The next thing you'll see on your side is the welcome pack from your own solicitor.

→ View your portal
```

---

### PM1 — Buyer has instructed their solicitor

**Vendor — to the seller:**

**Subject:** The buyer's solicitor is instructed, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's solicitor is instructed. Both sides are now formally in motion.

The buyer's next steps are their ID checks and getting funds on account with their solicitor, which covers the cost of searches and disbursements during conveyancing. The buyer is purchasing in cash, so there's no mortgage to arrange. That removes one of the most common sources of delay and uncertainty in a sale.

Nothing for you to do right now. The next thing you'll see on your side is the welcome pack from your solicitor.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Your solicitor's instructed, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor's instructed. That's the start of conveyancing on your purchase.

They'll be in touch shortly with their welcome pack: terms of business, ID requirements, and the first forms you'll need to fill in. Get those back as soon as you can. Your solicitor can't start substantive work until they have your ID cleared and your payment on account.

→ View your portal
```

---

## Phase 2 — Memorandum of sale

### VM2 — Seller has received the memorandum of sale

**Vendor — to the seller:**

**Subject:** Memorandum of sale issued, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The memorandum of sale has gone out. It's the document confirming the agreed price and the names of both parties, and it formally kicks off conveyancing on your side.

Your solicitor will now begin assembling the contract pack. If you've already had the welcome pack from your solicitor, getting that back is the single biggest thing you can do this week to keep the sale moving. If it hasn't landed yet, expect it imminently.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Memorandum of sale issued, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The memorandum of sale has gone out, confirming the agreed purchase price and the names of both parties. With it issued, conveyancing is now formally underway on both sides.

If you haven't already, return your solicitor's welcome pack and complete your ID checks. They can't get fully started on your purchase until those are in.

→ View your portal
```

---

### PM2 — Buyer has received the memorandum of sale

> No vendor email. Both parties hear about the MoS at the same trigger, and the vendor's already had VM2.

**Purchaser — to the buyer:**

**Subject:** Memorandum of sale issued, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The memorandum of sale is with your solicitor, confirming the agreed purchase price and the names of both parties. That formally kicks off conveyancing.

If you haven't already, return your solicitor's welcome pack and complete your ID checks. They can't get fully started until those are in.

→ View your portal
```

---

## Phase 3 — Welcome pack, ID/AML, money on account

### VM3 — Seller has received the welcome pack from their solicitor

**Vendor — to the seller:**

**Subject:** Welcome pack received from your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor's welcome pack has landed. That's their terms of business, the property questionnaire, and the ID requirements they need from you.

Get this back as quickly as you can. Your solicitor can't do substantive work on your sale until your ID is cleared, the AML checks are done, and the forms are returned. A few days is the right benchmark. Longer than that and the sale starts losing pace.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Seller is engaging with their solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller has their welcome pack from their solicitor. That's the kick-off paperwork for conveyancing on their side.

Nothing for you to do right now. The seller will return the forms to their solicitor in the coming days.

→ View your portal
```

---

### VM4 — Seller has completed ID and AML checks with their solicitor

**Vendor — to the seller:**

**Subject:** ID checks complete, 22 Example Road, London SW1A 1AA

```
Hi Alex,

ID and AML checks are done. That clears the way for your solicitor to begin substantive work.

They'll now move on to preparing the contract pack. Within the next week or two you'll receive the property information forms (TA6 and TA10). Fill them in promptly when they arrive. Delays at that stage are one of the main things that slow a sale down.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Seller's ID checks complete, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller's ID and AML checks are done. Their solicitor can now begin substantive work on the sale.

Nothing for you to do right now. The next thing you'll see on this trail is the contract pack arriving with your solicitor, likely a week or two away while the seller completes their property information forms.

→ View your portal
```

---

### PM3 — Buyer has completed ID and AML checks with their solicitor

**Vendor — to the seller:**

**Subject:** Buyer's ID checks complete, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's ID and AML checks are done. Their solicitor can now begin substantive work on the purchase.

Nothing for you to do right now. This is one of the early signals that things are moving properly on the buyer's side.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** ID checks complete, 22 Example Road, London SW1A 1AA

```
Hi Alex,

ID and AML checks are done. That clears the way for your solicitor to begin substantive work on your purchase.

If you haven't yet paid money on account to your solicitor, do that as soon as you can. They'll need it before they can order searches on your behalf.

→ View your portal
```

---

### PM4 — Buyer has paid money on account to their solicitor

**Vendor — to the seller:**

**Subject:** Buyer has put funds with their solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's funds are with their solicitor. That's the money on account that covers searches and disbursements during conveyancing. Buyers committing funds this early is one of the clearest signals they intend to proceed.

Searches will be ordered shortly, and that's usually the next major step on the buyer's side.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Payment on account received by your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has your payment on account. That covers searches and disbursements they'll incur on your behalf during conveyancing. It's separate from your deposit.

They can now order searches and proceed with the full conveyancing process. Searches take 2 to 4 weeks to come back depending on the local authority, so the sooner they're in, the sooner that piece is moving in the background.

→ View your portal
```

---

## Phase 4 — Property information forms

### VM5 — Seller has received the property information forms from their solicitor

**Vendor — to the seller:**

**Subject:** Property information forms received, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor needs information from you. The property information forms have arrived: TA6 (the general property questionnaire) and TA10 (fittings and contents).

Complete these as thoroughly and accurately as you can. They're legal documents and your answers become part of the contract. Return them to your solicitor promptly. If you're unsure about any question, call your solicitor before leaving anything blank rather than guessing.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Seller is gathering property information, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller has their property information forms from their solicitor. Those capture details about the property's history, what's included in the sale, and its planning history. Their answers go into the contract pack your solicitor will review.

Nothing for you to do right now. The seller will complete and return the forms in the coming days.

→ View your portal
```

---

### VM6 — Seller has returned completed property information forms to their solicitor

**Vendor — to the seller:**

**Subject:** Property forms returned to your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your completed property information forms are back with your solicitor. They'll now fold them into the contract pack alongside the title documents.

Once everything's assembled, the contract pack goes across to the buyer's solicitor, and the substantive review on their side begins.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** The seller has returned their property information forms, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller has returned their completed property information forms to their solicitor. Those will be included in the contract pack that comes to your solicitor.

Nothing to do from your side right now. The seller's solicitor will finalise the contract pack and send it across shortly.

→ View your portal
```

---

## Phase 5 — Mortgage

> 🚫 **All three mortgage milestones (PM5, PM6, PM11) are not applicable on a cash purchase.** No emails fire on this journey. [SHAPE: mortgage emails fire on mortgage journey only]

---

## Phase 6 — Draft contract pack (first bilateral phase)

> Bilateral milestone. Contract pack is *issued* by the seller's solicitor (VM7) and *received* by the buyer's solicitor (PM7). Variants cover natural-order and inverse-order confirmation across portal and internal routes.

---

### VM7 — Seller's solicitor has issued the draft contract pack

**Natural order, acted side — Portal route:**

**Subject:** You've confirmed the contract pack has gone out, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed that your solicitor's sent the draft contract pack across to the buyer's side.

The contract pack is the bundle that forms the legal foundation of the sale: the draft contract, the title documents, and the property information forms you completed (TA6 and TA10).

The buyer's solicitor will now review everything and raise enquiries about anything they want clarified. Their first round typically lands with your solicitor within a week or two. Your solicitor will handle the formal back-and-forth and come to you on any specific point that needs your read.

→ View your portal
```

**Natural order, acted side — Internal route:**

**Subject:** Contract pack issued to the buyer's side, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The draft contract pack has gone across to the buyer's solicitor. We've logged it on your sale.

The contract pack is the bundle that forms the legal foundation of the sale: the draft contract, the title documents, and the property information forms you completed (TA6 and TA10).

The buyer's solicitor will now review everything and raise enquiries with your solicitor within a week or two. Your solicitor will handle the formal back-and-forth and come to you on any specific point that needs your read.

→ View your portal
```

**Natural order, hand-off nudge to the buyer:**

**Subject:** Contract pack on its way to your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The draft contract pack has been issued by the seller's side, and it's on its way to your solicitor.

When your solicitor lets you know it's landed, open your portal and tap the highlighted confirm button. That logs receipt on the file and triggers the next steps. Takes about ten seconds.

→ Open your portal
```

**Inverse order, acted side — Portal route:**

**Subject:** You've confirmed the contract pack went out, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed your solicitor sent the contract pack across, and the buyer's side has already logged receipt on their end.

The contract pack is the bundle that forms the legal foundation of the sale: the draft contract, title documents, and the property forms you completed.

Because the buyer's solicitor already has it, they'll be starting their review now. Their first round of enquiries typically lands with your solicitor within a week or two.

→ View your portal
```

**Inverse order, acted side — Internal route:**

**Subject:** Contract pack issuance logged, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The draft contract pack has been issued by your solicitor. We've logged it on your sale. The buyer's side had already confirmed receipt before this came in, so the two are now in sync.

The buyer's solicitor will be starting their review now. Their first round of enquiries typically lands with your solicitor within a week or two.

→ View your portal
```

---

### PM7 — Buyer's solicitor has received the draft contract pack

**Natural order, acted side — Portal route:**

**Subject:** You've confirmed the contract pack has arrived, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed your solicitor has the draft contract pack from the seller's side.

That's the bundle of documents that forms the legal foundation of your purchase: the draft contract, title documents, and the seller's property information forms.

Your solicitor will now review everything carefully and raise enquiries with the seller's solicitor over the next week or two. While that's in flight, your survey is the other big piece worth keeping moving in parallel.

→ View your portal
```

**Natural order, acted side — Internal route:**

**Subject:** Contract pack received by your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor now has the draft contract pack from the seller's solicitor. We've logged it on your purchase.

That's the bundle of documents that forms the legal foundation of your purchase: the draft contract, title documents, and the seller's property information forms.

Your solicitor will now review everything and raise enquiries with the seller's solicitor over the next week or two. Keep your survey progressing in parallel.

→ View your portal
```

**Inverse order, acted side — Portal route:**

**Subject:** You've confirmed the contract pack has arrived, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed your solicitor has the contract pack from the seller's side, ahead of the seller's confirmation that it went out. Both sides are now in sync on the file.

Your solicitor will review the pack carefully and raise enquiries with the seller's solicitor over the next week or two. Keep your survey progressing in parallel.

→ View your portal
```

**Inverse order, acted side — Internal route:**

**Subject:** Contract pack receipt logged, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has the contract pack from the seller's side. We've logged it on your purchase, ahead of the seller's side logging that it went out.

Your solicitor will now review everything and raise enquiries with the seller's solicitor over the next week or two. Keep your survey progressing in parallel.

→ View your portal
```

**Inverse-direction hand-off to the seller:**

**Subject:** Buyer's side has confirmed receipt of the contract pack, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's side has logged receipt of the contract pack ahead of your side confirming it was issued. When you've spoken to your solicitor and they've confirmed it's gone out, open your portal and tap the highlighted confirm button to bring the two in sync. Takes about ten seconds.

→ Open your portal
```

---

## Phase 7 — Management pack

> 🚫 **Entire phase suppressed on freehold.** VM8, VM9, PM12 do not fire. [SHAPE: Phase 7 fires on leasehold only]

---

## Phase 8 — Searches

### PM8 — Buyer's solicitor has ordered searches

**Vendor — to the seller:**

**Subject:** Buyer's solicitor has ordered searches, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's solicitor has placed the property searches: the standard package of Local Authority, Drainage & Water, and Environmental, plus any area-specific ones. Buyers don't pay for searches until their solicitor has the funds in place, so this is one of the clearest commitment signals on the buyer's side.

Results take 2 to 4 weeks to come back. Your solicitor will field anything material that flows from them, and if a specific point needs your input (a planning matter, a drainage question, anything property-specific), they'll be in touch directly.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Searches ordered, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has ordered the property searches. The standard package covers the Local Authority search (planning, road adoption, restrictions, building control), a Drainage & Water search, and an Environmental search. Depending on the area, they may also order Chancel Repair, Coal Mining, or other location-specific searches.

Results take 2 to 4 weeks to come back, though the Local Authority search is often the slowest piece and can stretch longer in some areas. Your solicitor will review everything once it's all in and flag anything material. This one runs in the background while the enquiries side progresses.

→ View your portal
```

---

### PM13 — Buyer's solicitor has received the search results

**Vendor — to the seller:**

**Subject:** Buyer's search results are in, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's solicitor has the search results back. They'll review them alongside the contract pack to build the full picture of the property.

If anything material surfaces that needs clarification from your side, it'll come through as an enquiry, and your solicitor will reach you directly on any point that needs your input.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Search results received by your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has the search results back. They'll review them now alongside the contract pack.

The review typically takes a few days to a week. Any enquiries that flow from the results will land with the seller's solicitor soon after. If something material surfaces that needs your read or a decision, your solicitor will be in touch directly.

→ View your portal
```

---

## Phase 9 — Survey

### PM9 — Buyer has booked their property survey

**Vendor — to the seller:**

**Subject:** Buyer has booked their survey, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer has booked their property survey. The surveyor will be in touch directly to arrange access. A typical visit is 1 to 3 hours depending on whether it's a Level 2 (homebuyer) or Level 3 (full structural) inspection.

Nothing for you to do beyond letting the surveyor in when they come. The report goes to the buyer directly, and any findings that affect the sale will come back to your solicitor as enquiries.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Your survey is booked, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your property survey is booked. The surveyor will go through the property in person and produce their report directly to you. A Level 2 (homebuyer) typically takes 1 to 2 hours on site; a Level 3 (full structural) can take 2 to 3.

The report usually lands within a week or two of the visit. Once you have it, read it carefully. If anything looks material, call your surveyor before deciding what to do. They've seen enough properties to give you a real sense of whether a finding is meaningful or routine.

→ View your portal
```

---

### PM10 — Buyer has received the survey report

> Both sides notified. Vendor copy is existence-only — never describes report contents.

**Purchaser — to the buyer:**

**Subject:** Your survey report is in, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your survey report has landed.

Read it carefully. The surveyor's findings are organised by traffic-light condition ratings: green is fine, amber is something to be aware of, red is something to act on. Focus on the reds and the ambers. Call your surveyor on anything you don't understand or aren't sure how to weigh. They'll usually explain over the phone, and they can tell you whether a specific finding is genuinely material or whether it's a standard observation for a property of this type.

If anything from the report needs to be raised with the seller's side (a defect that affects price or completion, evidence of something the property forms didn't mention), your solicitor is the right route. Decide first what you actually want to ask for, then talk to your solicitor about how to put it across.

→ View your portal
```

**Vendor — to the seller:** *(added Job B 2026-05-29)*

**Subject:** Buyer's survey report is in, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's survey report has landed with them.

The surveyor has produced their report and it's now with the buyer to read and review. No news is generally good news on a survey report. If anything in it does need to be raised with your side, your solicitor may be asked for documents through the normal enquiry process, and we'll be in touch directly if there's anything else from our side that needs your input.

→ View your portal
```

---

## Phase 10 — Initial enquiries (bilateral)

> Bilateral round. PM14 raises, VM10 receives, VM11 (unilateral) is the seller giving input, VM12 issues the formal replies, PM15 receives them, PM16 (unilateral) is the buyer-side review.

---

### PM14 — Buyer's solicitor has raised initial enquiries

**Natural order, acted side — Portal route:**

**Subject:** You've confirmed the initial enquiries have gone out, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed your solicitor has raised the initial enquiries with the seller's side.

Enquiries are the formal questions your solicitor sends after reading the contract pack. They cover anything that needs clarifying about title, planning, what's included in the sale, and anything in the seller's property forms that needs more detail.

Typical turnaround for the first round is 1 to 4 weeks. Your solicitor will surface anything material once the replies come back.

→ View your portal
```

**Natural order, acted side — Internal route:**

**Subject:** Initial enquiries raised on your purchase, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has raised the initial enquiries with the seller's side. We've logged it on your purchase.

Enquiries are the formal questions your solicitor sends after reading the contract pack. They cover anything that needs clarifying about title, planning, what's included in the sale, and anything in the seller's property forms that needs more detail.

Turnaround for the first round is 1 to 4 weeks. Your solicitor will surface anything material once the replies are back.

→ View your portal
```

**Natural order, hand-off nudge to the seller:**

**Subject:** Initial enquiries on the way, please confirm receipt, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's solicitor has raised the initial enquiries, and they're on their way to your solicitor.

When your solicitor lets you know they've landed, open your portal and tap the highlighted confirm button. That logs receipt on the file and triggers the next steps. Takes about ten seconds.

→ Open your portal
```

**Inverse order, acted side — Portal route:**

**Subject:** You've confirmed the initial enquiries have gone out, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed your solicitor has raised the initial enquiries, and the seller's side has already logged receipt on their end.

Both sides are now in sync. The seller's solicitor will work through the questions with the seller. Typical turnaround for the first round is 1 to 4 weeks.

→ View your portal
```

**Inverse order, acted side — Internal route:**

**Subject:** Initial enquiries raise logged, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has raised the initial enquiries with the seller's side. We've logged it on your purchase. The seller's side had already confirmed receipt before this came in, so the two are now in sync.

Typical turnaround for the first round is 1 to 4 weeks. Your solicitor will surface anything material once the replies are back.

→ View your portal
```

---

### VM10 — Seller's solicitor has received initial enquiries

**Natural order, acted side — Portal route:**

**Subject:** You've confirmed your solicitor has the enquiries, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed the initial enquiries are with your solicitor, landed from the buyer's side.

Enquiries are the formal questions the buyer's solicitor raises after reading the contract pack. They cover anything that needs clarifying about title, planning, what's included in the sale, and anything in your property forms that needs more detail. They're a normal part of every conveyancing process, not a sign anything's wrong.

Your solicitor will work through the list and come to you for input on points only you can answer: works you've had done, neighbour relationships, planning consents you've used, anything where the property's history matters. Try to respond promptly when they reach out. The file moves at the speed of these answers.

→ View your portal
```

**Natural order, acted side — Internal route:**

**Subject:** Buyer's enquiries received by your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has received the initial enquiries from the buyer's solicitor. We've logged it on your sale.

Enquiries are the formal questions the buyer's solicitor raises after reading the contract pack. They cover title, planning, what's included in the sale, and anything in your property forms that needs more detail. They're a normal part of every conveyancing process.

Your solicitor will work through the list and come to you for input on points only you can answer. Try to respond promptly when they do. The file moves at the speed of these answers.

→ View your portal
```

**Inverse-direction hand-off to the buyer:**

**Subject:** Seller's side has confirmed receipt of the enquiries, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller's side has logged receipt of your solicitor's initial enquiries, ahead of your side confirming they went out. When your solicitor confirms the issuance to you, open your portal and tap the highlighted confirm button to bring the two in sync. Takes about ten seconds.

→ Open your portal
```

---

### VM11 — Seller has provided initial replies to their solicitor

> Unilateral. Seller-side input action.

**Vendor — to the seller:**

**Subject:** Your input on the enquiries is in, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your part of the enquiry response is done. The answers and information you sent to your solicitor cover the points the buyer's side raised in the first round.

Your solicitor will now compile your input into the formal written replies and send them across to the buyer's solicitor, typically within a few days.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Seller has provided their input on the enquiries, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Movement on the enquiries. The seller has given their solicitor the input needed to draft the formal replies, which will then come across to your solicitor.

Expect the replies to land in the next few days.

→ View your portal
```

---

### VM12 — Seller's solicitor has issued initial responses to the buyer's solicitor

**Natural order, acted side — Portal route:**

**Subject:** You've confirmed the replies have gone out, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed your solicitor has sent the formal replies to the buyer's side.

The first round of enquiry handling is now in the buyer's solicitor's hands. They'll review your replies over the next few days to a week. If your answers satisfy them, the file moves on. If anything's left open, they'll come back with follow-up enquiries, and your solicitor will handle those.

→ View your portal
```

**Natural order, acted side — Internal route:**

**Subject:** Replies to enquiries issued by your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has issued the formal replies to the buyer's side. We've logged it on your sale.

The first round of enquiry handling is now in the buyer's solicitor's hands. They'll review your replies over the next few days to a week. If your answers satisfy them, the file moves on. If anything's left open, they'll come back with follow-up enquiries.

→ View your portal
```

**Natural order, hand-off nudge to the buyer:**

**Subject:** Replies to your enquiries are on the way, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller's solicitor has issued the formal replies to your initial enquiries, and they're on their way to your solicitor.

When your solicitor confirms they've landed, open your portal and tap the highlighted confirm button. Takes about ten seconds.

→ Open your portal
```

**Inverse order, acted side — Portal route:**

**Subject:** You've confirmed the replies have gone out, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed your solicitor sent the formal replies, and the buyer's side has already logged receipt on their end.

Both sides are now in sync. The buyer's solicitor will review the replies over the next few days to a week.

→ View your portal
```

**Inverse order, acted side — Internal route:**

**Subject:** Replies-issuance logged, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has issued the formal replies to the buyer's side. We've logged it on your sale, ahead of the buyer-side having logged receipt.

The buyer's solicitor will review the replies over the next few days to a week.

→ View your portal
```

---

### PM15 — Buyer's solicitor has received initial replies from the seller's solicitor

**Natural order, acted side — Portal route:**

**Subject:** You've confirmed the replies have arrived, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. The seller's formal replies are now with your solicitor.

Your solicitor will work through them carefully, checking that each answer satisfies the question raised, flagging anything unclear or left open, and identifying anything that needs follow-up.

The review usually takes a few days to a week. Your solicitor will be in touch with anything material once they've worked through it.

→ View your portal
```

**Natural order, acted side — Internal route:**

**Subject:** Replies to your enquiries received by your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has received the seller's formal replies. We've logged it on your purchase.

They'll work through the replies carefully, checking each answer against the question raised, flagging anything unclear, and identifying anything that needs follow-up. The review usually takes a few days to a week.

→ View your portal
```

**Inverse-direction hand-off to the seller:**

**Subject:** Buyer's side has confirmed receipt of the replies, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's side has logged receipt of the formal replies ahead of your side confirming they went out. When your solicitor confirms the issuance, open your portal and tap the highlighted confirm button to bring the two in sync. Takes about ten seconds.

→ Open your portal
```

---

### PM16 — Buyer's solicitor has reviewed the initial replies

> Unilateral. Buyer-side review.

**Purchaser — to the buyer:**

**Subject:** Your solicitor has reviewed the seller's replies, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has finished reviewing the seller's replies.

A review of this kind usually lands in one of three places. Either the replies satisfied everything and your solicitor's comfortable to move the file forward, or some points need follow-up and additional enquiries will be raised, or something material has surfaced that warrants a conversation with you before deciding how to proceed.

Your solicitor will be in touch about which applies to your file. If it's all clear, the file moves to the next stage. If follow-up enquiries are needed, expect that to add a couple of weeks to the timeline. If a material concern has surfaced, your solicitor will walk you through what they've found and what your options are.

→ View your portal
```

---

## Phase 11 — Further enquiries (bilateral)

> Same bilateral shape as Phase 10, second round.

---

### PM17 — Buyer's solicitor has raised additional enquiries

**Natural order, acted side — Portal route:**

**Subject:** You've confirmed the follow-up enquiries have gone out, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed your solicitor has raised the follow-up enquiries with the seller's side.

After reviewing the initial replies, your solicitor flagged specific points that needed more follow-up. The follow-up round is typically a shorter, more focused list than the initial enquiries, and turnaround is usually faster: 1 to 3 weeks.

→ View your portal
```

**Natural order, acted side — Internal route:**

**Subject:** Additional enquiries raised on your purchase, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has raised the follow-up enquiries with the seller's side. We've logged it on your purchase.

After reviewing the initial replies, your solicitor flagged specific points that needed tightening up. The follow-up round is typically a shorter list and turns around faster: 1 to 3 weeks.

→ View your portal
```

**Natural order, hand-off nudge to the seller:**

**Subject:** Follow-up enquiries on the way, please confirm receipt, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Round 2 of enquiries is inbound. The buyer's solicitor has raised them today, and they're heading to your solicitor.

When your solicitor lets you know they've landed, open your portal and tap the highlighted confirm button. Takes about ten seconds.

→ Open your portal
```

**Inverse order, acted side — Portal route:**

**Subject:** You've confirmed the follow-up enquiries have gone out, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed your solicitor has raised the follow-up enquiries, and the seller's side has already logged receipt.

Both sides are now in sync. Turnaround on the second round is typically faster: 1 to 3 weeks.

→ View your portal
```

**Inverse order, acted side — Internal route:**

**Subject:** Follow-up enquiries raise logged, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has raised the follow-up enquiries with the seller's side. We've logged it on your purchase. The seller-side had already confirmed receipt, so the two are now in sync.

Turnaround on the second round is typically faster: 1 to 3 weeks.

→ View your portal
```

---

### VM13 — Seller's solicitor has received additional enquiries

**Natural order, acted side — Portal route:**

**Subject:** You've confirmed the follow-up enquiries have landed, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. Your solicitor now has the buyer's follow-up enquiries.

These are the buyer's solicitor's follow-up questions after working through the initial replies. Typically a shorter list focused on specific points the first round didn't fully close out.

Your solicitor will work through them and come to you if any need fresh input. Turnaround on follow-up rounds is usually faster than the first, so this should move more quickly.

→ View your portal
```

**Natural order, acted side — Internal route:**

**Subject:** Buyer's follow-up enquiries received by your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The follow-up enquiries are now with your solicitor. We've logged it on your sale.

These are the buyer's solicitor's follow-up questions after working through the initial replies. Typically a shorter, more focused list. Your solicitor will work through them and come to you if any need fresh input.

→ View your portal
```

**Inverse-direction hand-off to the buyer:**

**Subject:** Seller's side has confirmed receipt of the follow-ups, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller's side has logged receipt of the follow-up enquiries ahead of your side confirming they went out. When your solicitor confirms the issuance, open your portal and tap the highlighted confirm button to bring the two in sync.

→ Open your portal
```

---

### VM14 — Seller has provided additional replies to their solicitor

> Unilateral.

**Vendor — to the seller:**

**Subject:** Your input on the follow-ups is in, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your part on the follow-up enquiries is done. Your solicitor will now compile your input into the formal written replies and send them across.

Same rhythm as round one. A few days for the compilation, then the replies go to the buyer's side.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Seller has provided their input on the follow-ups, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller has given their solicitor the input for the follow-up replies, which will then come across to your solicitor.

Expect them to land in the next few days.

→ View your portal
```

---

### VM15 — Seller's solicitor has issued additional responses to the buyer's solicitor

**Natural order, acted side — Portal route:**

**Subject:** You've confirmed the follow-up replies have gone out, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed your solicitor has sent the formal follow-up replies to the buyer's side.

The buyer's solicitor will review them and conclude where the enquiry side of the file lands. Typically faster than the first round because there are fewer points to consider.

→ View your portal
```

**Natural order, acted side — Internal route:**

**Subject:** Follow-up replies issued by your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has issued the follow-up replies to the buyer's side. We've logged it on your sale.

The buyer's solicitor will review them and conclude where the enquiry side of the file lands. Typically faster than the first round.

→ View your portal
```

**Natural order, hand-off nudge to the buyer:**

**Subject:** Follow-up replies on the way to your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The follow-up replies are on their way to your solicitor, issued by the seller's side today.

When your solicitor confirms they've landed, open your portal and tap the highlighted confirm button. Takes about ten seconds.

→ Open your portal
```

**Inverse order, acted side — Portal route:**

**Subject:** You've confirmed the follow-up replies have gone out, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed your solicitor sent the follow-up replies, and the buyer's side has already logged receipt.

Both sides are in sync. The buyer's solicitor will review and conclude on the enquiry side of the file.

→ View your portal
```

**Inverse order, acted side — Internal route:**

**Subject:** Follow-up replies issuance logged, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has issued the follow-up replies. We've logged it on your sale, ahead of the buyer-side having logged receipt.

The buyer's solicitor will review and conclude on the enquiry side of the file.

→ View your portal
```

---

### PM18 — Buyer's solicitor has received additional replies

**Natural order, acted side — Portal route:**

**Subject:** You've confirmed the follow-up replies have arrived, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. The seller's follow-up replies are now with your solicitor.

Your solicitor will work through them and let you know what they conclude. Typically faster than the initial review since there are fewer points to consider.

→ View your portal
```

**Natural order, acted side — Internal route:**

**Subject:** Follow-up replies received by your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller's follow-up replies have landed with your solicitor. We've logged it on your purchase.

Your solicitor will work through them and let you know what they conclude. Typically faster than the initial review.

→ View your portal
```

**Inverse-direction hand-off to the seller:**

**Subject:** Buyer's side has confirmed receipt of the follow-up replies, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's side has logged receipt of the follow-up replies ahead of your side confirming they went out. When your solicitor confirms the issuance, open your portal and tap the highlighted confirm button to bring the two in sync.

→ Open your portal
```

---

### PM19 — Buyer's solicitor has reviewed the additional replies

> Unilateral.

**Purchaser — to the buyer:**

**Subject:** Your solicitor has reviewed the follow-up replies, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has finished reviewing the follow-up replies.

By this stage in the enquiry process, the most common landing is that the follow-up replies satisfy everything and the file moves on to the final report and contract sign-off. Occasionally one or two points remain open and a tight third round of enquiries is needed. That's unusual this late but not unheard of. Very occasionally something material surfaces that warrants a serious conversation about how to proceed.

Whichever applies, your solicitor will come to you directly. If it's material concerns, expect a proper sit-down conversation rather than just an email update.

→ View your portal
```

---

### PM20 — Buyer's solicitor has confirmed all enquiries are now satisfied

**Vendor — to the seller:**

**Subject:** Enquiries are all satisfied, 22 Example Road, London SW1A 1AA

```
Hi Alex,

A real moment on your sale. The buyer's solicitor has confirmed all enquiries are satisfied.

The buyer's side is now comfortable with the answers they've received. Your side's work on the enquiries is fully complete. No more questions coming back from this round or any future round on this file.

The file moves to contract sign-off on both sides. The remaining steps are more procedural: final reports, contracts, deposit, exchange. The pace typically picks up once the enquiries phase is closed.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Enquiries are all satisfied, 22 Example Road, London SW1A 1AA

```
Hi Alex,

A real moment on your purchase. All enquiries are now satisfied.

Your solicitor has confirmed they're comfortable with the answers and that nothing else is outstanding on the enquiry side. The heaviest stretch of conveyancing, combing through the contract pack and raising and resolving questions, is behind you.

The file now moves to the final report stage, where your solicitor pulls together everything they've learned about the property and walks you through it before exchange. The remaining steps are real but procedural: final report, contract sign-off, deposit, exchange.

→ View your portal
```

---

## Phase 12 — Final report and contract sign-off

### PM21 — Buyer has received the final report from their solicitor

**Vendor — to the seller:**

**Subject:** Buyer has the final report, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer has the final report from their solicitor. That's the structured summary of everything that came out of the conveyancing process. The buyer is now in the reading-and-deciding window before contracts go out for signing.

Contract documents typically follow within a few days of the buyer being comfortable with the report. Your side's contract documents will be issued by your solicitor in parallel.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Your final report is in, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor's final report has landed.

The report is your solicitor's structured summary of everything they've found out about the property: title, planning, the search results, and anything material that surfaced along the way. It's not new investigation. It's the digest.

Read it carefully. This is your considered moment before signing the contracts. Write down anything that's unclear or you want to talk through, and call your solicitor before you sign rather than after. Once you're comfortable, your solicitor will issue the contracts for you to sign.

→ View your portal
```

---

### VM16 — Seller's solicitor has issued contract documents to the seller

**Vendor — to the seller:**

**Subject:** Contracts ready for your signature, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your contracts are ready for signing. Your solicitor has issued the contract pack to you: the final draft contract, the transfer deed, and any other documents that need your signature. This is the formal signing step before exchange.

Read through everything before signing. Your solicitor should have walked you through anything notable, but check the price, the names, the property address, and any agreed inclusions or exclusions match what you're expecting. Sign and return promptly. Your solicitor will hold the signed documents in readiness for exchange.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Seller is signing the contracts, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller's contracts are with them for signing. Their solicitor has sent the contract documents to the seller, and will hold the signed documents in readiness once they're back.

→ View your portal
```

---

### VM17 — Seller's solicitor has received signed contract documents back from the seller

**Vendor — to the seller:**

**Subject:** Signed contracts received by your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your signed contracts are back with your solicitor. Your active part of the contract sign-off is complete. Your solicitor will hold the signed documents in readiness. Once both sides are signed, the two solicitors can begin coordinating the actual exchange moment.

→ View your portal
```

**Purchaser — to the buyer:** *(added Job B 2026-05-29; closing line varies by buyer funding)*

**Subject:** Seller has signed the contracts, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller has signed their contracts and returned them to their solicitor.

That's the seller's half of contract sign-off complete. Their solicitor is now holding the signed documents in readiness. Once your own contracts are signed and back with your solicitor, both sides will be ready to begin coordinating the actual exchange moment. From here, the file moves at the pace of your contract sign-off and your deposit reaching your solicitor.

→ View your portal
```

> Cash-from-proceeds variant of the closing line: `...the pace of your contract sign-off and exchange readiness on your related sale.` See `journey-freehold-cash_from_proceeds-FINAL.md` for the CFP-specific rendering.

---

### PM22 — Buyer has received contract documents from their solicitor

**Vendor — to the seller:**

**Subject:** Buyer has the contracts, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer now has their contract documents from their solicitor. Like you, they're in their signing window. Once their signed documents are back with their solicitor, both sides are ready to begin coordinating exchange.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Contracts ready for your signature, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your contracts are ready for signing. Your solicitor has issued the contract pack to you: the final draft contract, the transfer deed, and any other documents that need your signature. This is the formal signing step before exchange.

Read through everything before signing. Your solicitor should have walked you through anything notable, but check the price, the names, the property address, and any agreed inclusions or exclusions match what you're expecting. Sign and return promptly. Your solicitor will hold the signed documents in readiness for exchange.

→ View your portal
```

---

### PM23 — Buyer's solicitor has received signed contract documents back from the buyer

**Vendor — to the seller:**

**Subject:** Buyer's signed contracts received by their solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's signed contracts are back with their solicitor. Both sides are now signed and held in readiness. The two solicitors will coordinate the exchange moment from here.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Your signed contracts are back with your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your signed contracts are back with your solicitor. Your active part of the contract sign-off is complete. Your solicitor will hold the signed documents in readiness and finalise the exchange coordination with the seller's side.

The remaining step before exchange is your deposit reaching your solicitor, ready to transfer on exchange. Your solicitor will let you know the amount and timing if they haven't already.

→ View your portal
```

---

## Phase 13 — Deposit

> Fires on cash and mortgage buyers. The buyer transfers the deposit (usually 10% of the price) to their solicitor, ready to be released on exchange. [SHAPE: suppressed on cash-from-proceeds, where the deposit comes from the related sale's equity]

---

### PM24 — Buyer has transferred the deposit to their solicitor

**Vendor — to the seller:**

**Subject:** Buyer's deposit is with their solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's deposit is now with their solicitor, ready to be released on exchange. That's one of the strongest commitment signals in the whole process. Money is committed and in place.

With the deposit held and contracts signed on both sides, the remaining step is the two solicitors agreeing the exchange moment. Exchange is close.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Your deposit is with your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your deposit is now with your solicitor, ready for exchange.

This is the deposit (usually around 10% of the purchase price) that's released to the seller's side at the moment of exchange. It's held safely by your solicitor until then. The balance of the purchase price follows at completion.

With your deposit in place and your contracts signed, you're ready for exchange. Your solicitor will coordinate the exact moment with the seller's side.

→ View your portal
```

---

## Phase 14 — Ready to exchange

> Agent-confirm only. No portal-route variants.

---

### VM18 — Seller's solicitor has confirmed readiness to exchange

**Vendor — to the seller:**

**Subject:** Your solicitor is ready to exchange, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has confirmed they're ready to exchange.

Everything on your side is in place: contracts signed and held in readiness, conveyancing complete, completion date agreed. Your solicitor has formally confirmed they're ready to proceed to exchange whenever the buyer's side is also there.

Exchange happens when both solicitors are ready and agree the moment. If the buyer's side is also there, exchange is imminent. If they're not yet, it follows as soon as they confirm. Either way, the next time you hear from us, exchange will have happened, and that's the legal commitment moment.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Seller's side is ready to exchange, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller's solicitor has formally confirmed everything on their side is in place. Once both sides have formally confirmed, exchange follows. It's a procedural decision between the two solicitors at that point.

→ View your portal
```

---

### PM25 — Buyer's solicitor has confirmed readiness to exchange

**Vendor — to the seller:**

**Subject:** Buyer's side is ready to exchange, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's solicitor has formally confirmed everything on their side is in place: contracts signed, conveyancing complete, deposit transferred and held ready. Once both sides have formally confirmed, exchange follows.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Your solicitor is ready to exchange, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has confirmed they're ready to exchange.

Everything on your side is in place: contracts signed and held in readiness, conveyancing complete, deposit transferred and ready, completion date agreed. Your solicitor has formally confirmed they're ready to proceed to exchange whenever the seller's side is also there.

Exchange happens when both solicitors are ready and agree the moment. The next time you hear from us, exchange will have happened.

→ View your portal
```

---

## Phase 15 — Exchange

> Agent-confirm only. No portal-route variants.

---

### VM19 — Seller has received confirmation that contracts have exchanged

**Vendor — to the seller:**

**Subject:** Contracts have exchanged, your sale is now legally binding, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Exchange has happened. Your sale is now legally binding.

The two solicitors have formally exchanged contracts. The buyer's deposit is now released to the seller's side, and the agreed completion date is locked in. Neither party can pull out without significant legal and financial consequences. The sale is essentially certain at this point.

The next major moment is completion: when the balance funds transfer, ownership formally moves to the buyer, and keys are handed over. Closer to the date, expect another email with completion-day specifics: key handover, meter readings, vacating the property.

→ View your portal
```

---

### PM26 — Buyer has received confirmation that contracts have exchanged

**Purchaser — to the buyer:**

**Subject:** Contracts have exchanged, your purchase is now legally binding, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Exchange has happened. Your purchase is now legally binding.

The two solicitors have formally exchanged contracts. Your deposit has transferred to the seller's side, and the agreed completion date is locked in. You're now contractually entitled to buy the property on that date, and the seller is contractually obliged to sell.

The next big moment is completion: the balance funds transfer to the seller, ownership formally becomes yours, and the keys are handed over. Your solicitor will be in touch about transferring the balance funds for completion day. Closer to the date, expect another email with completion-day specifics: timing on the day, where to collect keys, what to do about utilities.

→ View your portal
```

---

## Phase 16 — Completion

> Agent-confirm only. No portal-route variants.

---

### VM20 — Seller has received confirmation that the sale has completed

**Vendor — to the seller:**

**Subject:** Completion, your sale is done, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Completion has happened. Your sale is done.

The balance funds have transferred to your solicitor, ownership has formally moved to the buyer, and the keys have been handed over. The legal and financial side of selling your property is now finalised. Your solicitor will be in touch shortly with the completion statement showing how the funds have been distributed, and the net amount that's coming to you.

From our side, this is where we step back. If anything comes up post-completion (utility queries, post-handover questions, anything where you need a pointer), do get in touch. Thank you for trusting us with your sale.

→ View your portal
```

---

### PM27 — Buyer has received confirmation that the sale has completed

**Purchaser — to the buyer:**

**Subject:** Completion, the property is yours, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Completion has happened. The property is yours.

The balance funds have transferred to the seller's side, ownership has formally moved across to you, and the keys are now yours. Everything from the offer-accepted moment to this one, the conveyancing, the enquiries, the searches, the survey, the contracts, all of it was the work that gets you to here.

Your solicitor will register the transfer with the Land Registry over the coming weeks. That's the formal legal record-keeping step, behind the scenes, no action from you. Practically: pick up the keys, switch the utilities into your name (your solicitor's completion statement will have the meter readings taken on the day), update your address with the bank, employer, DVLA, and the rest. Get the locks changed if you're inclined.

From our side, this is where we step back. Thank you for letting us help you through it. It's a real moment, and we hope the place becomes everything you wanted it to be.

→ View your portal
```

---

**End of journey: Freehold × Cash buyer.**
