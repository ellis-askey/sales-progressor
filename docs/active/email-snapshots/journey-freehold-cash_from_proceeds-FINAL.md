# Journey: Freehold × Cash from proceeds

Final rewritten email matrix for this journey shape. All phases, every milestone, every variant.

**Voice:** Agency white-labelled "we". Transactional notification style. "Hi [name]" opener, no sign-off, ends at portal link.
**Lexicon:** "related sale" (not "concurrent"). Plain "has to exchange before this purchase can". No em dashes in body copy.
**Shape:** Freehold (no TA7, no management pack, Phase 7 suppressed). Cash from proceeds (mortgage suppressed, deposit transfer suppressed, chain dependency retained). See `SHAPE-NOTES.md`.

---

## Phase 1 — Instruction

### VM1 — Seller has instructed their solicitor

**Vendor — to the seller:**

**Subject:** Your solicitor's instructed, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor's instructed. That's conveyancing formally underway on your side.

They'll start preparing the contract pack now: title documents, the property forms you'll need to fill in (TA6 and TA10), and anything else that needs to go to the buyer's solicitor. The next thing you'll see from them is their welcome pack landing in the next few days.

When the welcome pack arrives, turning it around quickly is the single biggest thing you can do to keep the sale moving early on.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** The seller's solicitor is instructed, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller's solicitor is instructed. Conveyancing is now underway on their side.

They'll start preparing the contract pack: title documents and the property information forms. Once it's assembled, it goes across to your solicitor to review.

Nothing for you to do right now. Because you're funding this purchase from your own sale's equity, the two transactions are running side by side from here. We'll keep you posted on both, and your solicitor will be coordinating the timing across them.

→ View your portal
```

---

### PM1 — Buyer has instructed their solicitor

**Vendor — to the seller:**

**Subject:** The buyer's solicitor is instructed, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's solicitor is instructed. Both sides are now formally in motion.

The buyer's next steps are their ID checks and getting funds on account with their solicitor, which covers the cost of searches and disbursements during conveyancing. They're funding this purchase from their own sale's equity, so their related sale is running alongside this one, and their solicitor will be coordinating both transactions.

Nothing for you to do right now. The next thing you'll see on your side is the welcome pack from your solicitor.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Your solicitor's instructed, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor's instructed. That's the start of conveyancing on your purchase.

They'll be in touch shortly with their welcome pack: terms of business, ID requirements, and the first forms you'll need to fill in.

Because your deposit comes from your related sale, that sale has to exchange before this purchase can. The two are tied together at that point. Keep us posted on how the related sale is progressing. Your solicitor will be coordinating the timing of both, and the more visibility everyone has on the other side, the smoother that coordination is.

→ View your portal
```

---

## Phase 2 — Memorandum of sale

### VM2 — Seller has received the memorandum of sale

**Vendor — to the seller:**

**Subject:** Memorandum of sale issued, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The memorandum of sale has gone out. It's the document confirming the agreed price and the names of both parties, and it formally kicks off conveyancing on your side.

Your solicitor will now begin assembling the contract pack. If you've already had the welcome pack from your solicitor, getting that back is the single biggest thing you can do this week to keep the sale moving. If it hasn't landed yet, expect it imminently.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Memorandum of sale issued, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The memorandum of sale has gone out, confirming the agreed purchase price and the names of both parties. With it issued, conveyancing is now formally underway on both sides.

If you haven't already, return your solicitor's welcome pack and complete your ID checks. They can't get fully started on your purchase until those are in.

A reminder on your related sale: it has to exchange before this purchase can. Keep us posted on how it's progressing, and your solicitor will be coordinating timing across both.

→ View your portal
```

---

### PM2 — Buyer has received the memorandum of sale

**Purchaser — to the buyer:**

**Subject:** Memorandum of sale issued, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The memorandum of sale is with your solicitor, confirming the agreed purchase price and the names of both parties. That formally kicks off conveyancing.

If you haven't already, return your solicitor's welcome pack and complete your ID checks. They can't get fully started until those are in.

Your related sale matters here too: it has to exchange before this purchase can. Keep us posted, and your solicitor will coordinate the timing of both.

→ View your portal
```

---

## Phase 3 — Welcome pack, ID/AML, money on account

### VM3 — Seller has received the welcome pack from their solicitor

**Vendor — to the seller:**

**Subject:** Welcome pack received from your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor's welcome pack has landed. That's their terms of business, the property questionnaire, and the ID requirements they need from you.

Get this back as quickly as you can. Your solicitor can't do substantive work on your sale until your ID is cleared, the AML checks are done, and the forms are returned. A few days is the right benchmark. Longer than that and the sale starts losing pace.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Seller is engaging with their solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller has their welcome pack from their solicitor. That's the kick-off paperwork for conveyancing on their side.

Nothing for you to do right now. The seller will return the forms to their solicitor in the coming days.

→ View your portal
```

---

### VM4 — Seller has completed ID and AML checks with their solicitor

**Vendor — to the seller:**

**Subject:** ID checks complete, 22 Example Road, London SW1A 1AA

```
Hi Alex,

ID and AML checks are done. That clears the way for your solicitor to begin substantive work.

They'll now move on to preparing the contract pack. Within the next week or two you'll receive the property information forms (TA6 and TA10). Fill them in promptly when they arrive. Delays at that stage are one of the main things that slow a sale down.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Seller's ID checks complete, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller's ID and AML checks are done. Their solicitor can now begin substantive work on the sale.

Nothing for you to do right now. The next thing you'll see on this trail is the contract pack arriving with your solicitor, likely a week or two away while the seller completes their property information forms.

→ View your portal
```

---

### PM3 — Buyer has completed ID and AML checks with their solicitor

**Vendor — to the seller:**

**Subject:** Buyer's ID checks complete, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's ID and AML checks are done. Their solicitor can now begin substantive work on the purchase.

Nothing for you to do right now. This is one of the early signals that things are moving properly on the buyer's side.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** ID checks complete, 22 Example Road, London SW1A 1AA

```
Hi Alex,

ID and AML checks are done. That clears the way for your solicitor to begin substantive work on your purchase.

If you haven't yet paid money on account to your solicitor, do that as soon as you can. They'll need it before they can order searches on your behalf.

→ View your portal
```

---

### PM4 — Buyer has paid money on account to their solicitor

**Vendor — to the seller:**

**Subject:** Buyer has put funds with their solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's funds are with their solicitor. That's the money on account that covers searches and disbursements during conveyancing. Buyers committing funds this early is one of the clearest signals they intend to proceed.

Searches will be ordered shortly, and that's usually the next major step on the buyer's side.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Payment on account received by your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has your payment on account. That covers searches and disbursements they'll incur on your behalf during conveyancing. It's separate from your deposit.

They can now order searches and proceed with the full conveyancing process. Searches take 2 to 4 weeks to come back depending on the local authority, so the sooner they're in, the sooner that piece is moving in the background.

→ View your portal
```

---

## Phase 4 — Property information forms

### VM5 — Seller has received the property information forms from their solicitor

**Vendor — to the seller:**

**Subject:** Property information forms received, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor needs information from you. The property information forms have arrived: TA6 (the general property questionnaire) and TA10 (fittings and contents).

Complete these as thoroughly and accurately as you can. They're legal documents and your answers become part of the contract. Return them to your solicitor promptly. If you're unsure about any question, call your solicitor before leaving anything blank rather than guessing.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Seller is gathering property information, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller has their property information forms from their solicitor. Those capture details about the property's history, what's included in the sale, and its planning history. Their answers go into the contract pack your solicitor will review.

Nothing for you to do right now. The seller will complete and return the forms in the coming days.

→ View your portal
```

---

### VM6 — Seller has returned completed property information forms to their solicitor

**Vendor — to the seller:**

**Subject:** Property forms returned to your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your completed property information forms are back with your solicitor. They'll now fold them into the contract pack alongside the title documents.

Once everything's assembled, the contract pack goes across to the buyer's solicitor, and the substantive review on their side begins.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** The seller has returned their property information forms, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller has returned their completed property information forms to their solicitor. Those will be included in the contract pack that comes to your solicitor.

Nothing to do from your side right now. The seller's solicitor will finalise the contract pack and send it across shortly.

→ View your portal
```

---

## Phase 5 — Mortgage

> 🚫 **All three mortgage milestones (PM5, PM6, PM11) are not applicable on cash-from-proceeds purchases.** No emails fire on this journey.

---

## Phase 6 — Draft contract pack (first bilateral phase)

### VM7 — Seller's solicitor has issued the draft contract pack

(Same as freehold-cash_buyer — VM7 vendor copy is funding-agnostic. See journey-freehold-cash_buyer-FINAL.md for verbatim text.)

### PM7 — Buyer's solicitor has received the draft contract pack

**Natural order, acted side — Portal route:**

**Subject:** You've confirmed the contract pack has arrived, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed your solicitor has the draft contract pack from the seller's side.

That's the bundle of documents that forms the legal foundation of your purchase: the draft contract, title documents, and the seller's property information forms.

Your solicitor will now review everything carefully and raise enquiries with the seller's solicitor over the next week or two. While that's in flight, your survey is the other big piece worth keeping moving in parallel.

A reminder on your related sale: it has to exchange before this purchase can. Keep us posted on how it's progressing.

→ View your portal
```

**Natural order, acted side — Internal route:**

**Subject:** Contract pack received by your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor now has the draft contract pack from the seller's solicitor. We've logged it on your purchase.

That's the bundle of documents that forms the legal foundation of your purchase: the draft contract, title documents, and the seller's property information forms.

Your solicitor will now review everything and raise enquiries with the seller's solicitor over the next week or two. Keep your survey progressing in parallel. And on your related sale: it has to exchange before this purchase can, so keep us posted on how that's moving.

→ View your portal
```

**Inverse order, acted side — Portal route:**

**Subject:** You've confirmed the contract pack has arrived, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Thanks. You've confirmed your solicitor has the contract pack from the seller's side, ahead of the seller's confirmation that it went out. Both sides are now in sync on the file.

Your solicitor will review the pack carefully and raise enquiries with the seller's solicitor over the next week or two. Keep your survey progressing in parallel, and keep us posted on your related sale, which has to exchange before this purchase can.

→ View your portal
```

**Inverse order, acted side — Internal route:**

**Subject:** Contract pack receipt logged, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has the contract pack from the seller's side. We've logged it on your purchase, ahead of the seller's side logging that it went out.

Your solicitor will now review everything and raise enquiries with the seller's solicitor over the next week or two. Keep your survey progressing in parallel, and keep us posted on your related sale, which has to exchange before this purchase can.

→ View your portal
```

**Inverse-direction hand-off to the seller:**

(Same as freehold-cash_buyer — hand-off is funding-agnostic.)

---

## Phase 7 — Management pack

> 🚫 **Entire phase suppressed on freehold.** VM8, VM9, PM12 do not fire.

---

## Phase 8 — Searches

(Same as freehold-cash_buyer — searches are funding-agnostic.)

---

## Phase 9 — Survey

(Same as freehold-cash_buyer — survey is funding-agnostic.)

---

## Phase 10 — Initial enquiries (bilateral)

(Same as freehold-cash_buyer — enquiries are funding-agnostic.)

---

## Phase 11 — Further enquiries (bilateral)

(Same as freehold-cash_buyer — funding-agnostic.)

### PM20 — Buyer's solicitor has confirmed all enquiries are now satisfied

(Same as freehold-cash_buyer — funding-agnostic.)

---

## Phase 12 — Final report and contract sign-off

### PM21 — Buyer has received the final report from their solicitor

(Same as freehold-cash_buyer.)

### VM16 / VM17 / PM22 / PM23

VM16, PM22 (vendor) — same as freehold-cash_buyer.

**VM17 — Purchaser (CFP-specific closing line, added Job B 2026-05-29):**

**Subject:** Seller has signed the contracts, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The seller has signed their contracts and returned them to their solicitor.

That's the seller's half of contract sign-off complete. Their solicitor is now holding the signed documents in readiness. Once your own contracts are signed and back with your solicitor, both sides will be ready to begin coordinating the actual exchange moment. From here, the file moves at the pace of your contract sign-off and exchange readiness on your related sale.

→ View your portal
```

**PM22 — Buyer has received contract documents from their solicitor (purchaser variant):**

**Subject:** Contracts ready for your signature, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your contracts are ready for signing. Your solicitor has issued the contract pack to you: the final draft contract, the transfer deed, and any other documents that need your signature. This is the formal signing step before exchange.

Read through everything before signing. Your solicitor should have walked you through anything notable, but check the price, the names, the property address, and any agreed inclusions or exclusions match what you're expecting. Sign and return promptly. Your solicitor will hold the signed documents in readiness for exchange.

A reminder on your related sale: it has to exchange before this purchase can. If your sale isn't yet at the signed-and-ready stage, that's the remaining piece in front of exchange. Your solicitor will coordinate the two transactions to exchange together.

→ View your portal
```

**PM23 — Buyer's signed contracts received (purchaser variant):**

**Subject:** Your signed contracts are back with your solicitor, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your signed contracts are back with your solicitor. Your active part of the contract sign-off is complete. Your solicitor will hold the signed documents in readiness and finalise the exchange coordination with the seller's side.

Exchange happens when both solicitors confirm everyone's ready: both sides signed, completion date agreed, and on your side, your related sale at the same signed-and-ready stage. Your solicitor coordinates both transactions and will let you know if your sale's timeline needs nudging to keep the two in step.

→ View your portal
```

---

## Phase 13 — Deposit

> 🚫 **PM24 (Buyer has transferred the deposit) is not applicable on cash-from-proceeds purchases.** No email fires.

---

## Phase 14 — Ready to exchange

### VM18 — Seller's solicitor has confirmed readiness to exchange

(Same as freehold-cash_buyer.)

### PM25 — Buyer's solicitor has confirmed readiness to exchange

**Vendor — to the seller:**

**Subject:** Buyer's side is ready to exchange, 22 Example Road, London SW1A 1AA

```
Hi Alex,

The buyer's solicitor has formally confirmed everything on their side is in place: contracts signed, conveyancing complete, deposit covered through the buyer's related sale. Once both sides have formally confirmed, exchange follows.

→ View your portal
```

**Purchaser — to the buyer:**

**Subject:** Your solicitor is ready to exchange, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Your solicitor has confirmed they're ready to exchange.

Everything on your side is in place: contracts signed and held in readiness, conveyancing complete, completion date agreed. Your solicitor has formally confirmed they're ready to proceed to exchange whenever the seller's side, and your related sale, are also there.

Exchange happens when both solicitors are ready and agree the moment. Because your purchase exchange has to be coordinated with your related sale's exchange, your solicitor will be working with both sets of solicitors to land all the moving pieces together. The next time you hear from us, exchange will have happened.

→ View your portal
```

---

## Phase 15 — Exchange

### VM19 — Seller has received confirmation that contracts have exchanged

(Same as freehold-cash_buyer.)

### PM26 — Buyer has received confirmation that contracts have exchanged

**Purchaser — to the buyer:**

**Subject:** Contracts have exchanged, your purchase is now legally binding, 22 Example Road, London SW1A 1AA

```
Hi Alex,

Exchange has happened. Your purchase is now legally binding.

The two solicitors have formally exchanged contracts. Your deposit, funded from your related sale's equity, has been accounted for, and the agreed completion date is locked in. You're now contractually entitled to buy the property on that date, and the seller is contractually obliged to sell.

The next big moment is completion: the balance funds transfer to the seller, ownership formally becomes yours, and the keys are handed over. Your solicitor will be in touch about coordinating the balance funds, most of which come from the proceeds of your related sale completing on the same day. Closer to the date, expect another email with completion-day specifics: timing on the day, where to collect keys, what to do about utilities.

→ View your portal
```

---

## Phase 16 — Completion

### VM20 / PM27

Same as freehold-cash_buyer — completion emails are funding-agnostic.

---

**End of journey: Freehold × Cash from proceeds.**
