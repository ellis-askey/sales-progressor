...
- **PR4 — motion / voice / polish pass.** ✅ shipped. Staggered card entrance (reuses `agent-section-in`, reduced-motion guard), route `loading.tsx` for the list and firm detail, voice gate confirmed (em-dashes only in comments), dropped a non-existent status label. Inline-style → primitive migration **grandfathered** (Law 19) rather than done blind without a visual baseline — logged in [POLISH_TBD.md](../../POLISH_TBD.md) along with the unpaginated "all"-scope directory and the deferred per-firm risk metric.
