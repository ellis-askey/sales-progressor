...
| Idea | Notes | Date |
|---|---|---|
| **Estimated-exchange-date change entry** | When an agent moves the estimated exchange date (the plain setter or the revise-overdue flow), a client-visible timeline entry is posted: "We've updated your estimated exchange date to {date}. This is our latest estimate and could still change as the sale moves forward." Shows on Overview "Latest updates" + the Updates page as an unattributed agency update (orange icon, no name), flagged "New since your last visit". Both sides. Passive only — no email/push. Fires **only when an existing stored date moves to a different day**; a first-ever set and clearing stay silent. Also switched the client-facing wording from "expected" to **"estimated"** (calendar button + this copy); the hero already reads "12-week target". Staging. | 2026-08-30 |
...
| **Client → solicitor follow-up sender** | Agreed in principle; **copy deck must be founder-approved before build**. Evolution of the shipped "Email your conveyancer" button: a state that knows when a step is with the client's OWN solicitor (per-step responsibility map + enquiry tracker), a proactive nudge timed 1–3 days before our internal reminder, and a pre-filled follow-up in the client's voice (mailto, agency CC'd). Templates not live-AI; "following up" wording gated on a real filed sent copy (fails safe). Plus a Command Centre opened-vs-sent usage view. Full spec: [`client-solicitor-followup-sender-SPEC.md`](client-solicitor-followup-sender-SPEC.md). | 2026-08-18 |
