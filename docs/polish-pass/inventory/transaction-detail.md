...
| **Overview — ViewChainButton** | A `agent-link` or `agent-btn-ghost-bordered` "View chain" button outside any card. | `components/chain/ViewChainButton.tsx` | Not read in detail. Button renders as entry point to ChainDrawer. | Read file; verify button uses `agent-link` or canonical ghost class. |
| **Sidebar — Progress card** | `glass-card p-4` (16px). Label `.agent-sidebar-label` "Progress". ProgressRing (80px) + status `agent-pill` + weeks elapsed `text-xs`. | `components/transaction/TransactionSidebar.tsx` (Progress section, lines 141–156) | `glass-card p-5` (20px). `.agent-sidebar-label` ✓. ProgressRing ✓. `agent-pill` ✓. Gap: outer padding `p-5` vs. polish `p-4`. | Change `p-5` → `p-4` on Progress card `glass-card` wrapper. |
| **Sidebar — Exchange Forecast card** | `glass-card px-4 py-3`. Label `.agent-sidebar-label` "Exchange Forecast". Date labels `text-xs text-slate-900/40`. Date values `text-xs font-semibold text-slate-900/90` (polish renders at ~11px). | `components/transaction/TransactionSidebar.tsx` (Exchange Forecast section, lines 158–231) | `glass-card p-5`. `.agent-sidebar-label` ✓. Date labels `text-xs text-slate-900/40` ✓. Date values `text-sm font-semibold` (14px) — larger than polish. Gap: padding `p-5` vs. `px-4 py-3`; date value text size. | Change `p-5` → `px-4 py-3`; change date value class from `text-sm font-semibold` → `text-xs font-semibold` throughout this card. |
...
| **Milestones tab — Exchange Ready Banner** | Amber/gold glass card replacing progress card when both sides exchange-ready. Prominent headline + subtext. | `components/milestones/MilestonePanel.tsx` (`exchangeReady` branch) | Not read in detail — internal to MilestonePanel. | Read MilestonePanel's `exchangeReady` JSX branch; verify it uses `glass-card` with amber styling consistent with polish amber card. |
...
| **To-Do tab — ManualTaskList** | `glass-card overflow-hidden rounded-[12px]`. CardHdr: "My tasks". Done-section expand: `agent-acc`/`agent-acc-in`. "With Sales Progressor" sub-section: separate card or divider. | `components/todos/ManualTaskList.tsx` | Not read in detail. | Read file; verify outer wrapper uses `glass-card overflow-hidden rounded-[12px]` + CardHdr pattern; wire `agent-acc`/`agent-acc-in` on done-section expand; wire `agent-reveal-in` on add-task form expand. |
...
| **Activity tab — ComposeEmail** | Compose card. "Send" button: primary canonical. "Cancel": `agent-btn-ghost-bordered`. No-sender banner: `agent-link` for settings link. | `components/verified-emails/ComposeEmail.tsx` | Not read in detail. | Read file; verify Send button class, Cancel class, and settings link class against canonical patterns. |
