...
| `PropertyHero` | `components/transaction/PropertyHero.tsx` | Match polish page | Warm variant for agent surface. Address, status, progress bar, back link |
| `PropertyFileTabs` | `components/transaction/PropertyFileTabs.tsx` | Match polish page | Tab bar: Overview / Milestones / Reminders / To-Do / Activity |
| `StatusControl` | `components/transaction/StatusControl.tsx` | Match polish page | Status badge + dropdown; director only. Uses `useOptimistic` |
| `TransactionSidebar` | `components/transaction/TransactionSidebar.tsx` | Match polish page | Right column: Progress ring, Exchange Forecast, Agent card, Price & Fees. Opens `EditSaleDetailsDrawer` |
| `EditSaleDetailsDrawer` | `components/transaction/EditSaleDetailsDrawer.tsx` | Match polish page | Portal right-hand drawer: edit property/price/fees/dates. Multi-step `PropSaveStage` state machine. Opens from sidebar "Edit details" |
| `NextMilestoneWidget` | `components/transaction/NextMilestoneWidget.tsx` | Match polish page | Overview tab widget: next milestone per side, quick Confirm/tab-link button |
| `RemindersWidget` | `components/transaction/RemindersWidget.tsx` | Match polish page | Overview tab widget: upcoming/overdue reminder summary with "View all →" |
| `RecentActivityWidget` | `components/transaction/RecentActivityWidget.tsx` | Match polish page | Overview tab widget: last 3 comms entries with "View all →" |
| `RiskScoreWidget` | `components/transaction/RiskScoreWidget.tsx` | Match polish page | Overview tab widget: risk score indicator, factor list. Display only |
| `FileHealthBanner` | `components/transaction/FileHealthBanner.tsx` | Match polish page | Overview tab: amber/red warning banner when reminders are overdue or file is behind. Returns null when no issues |
| `MosConfirmedNotice` | `components/transaction/MosConfirmedNotice.tsx` | Out of scope | Returns null. Fires toast on `?mosConfirmed=1`. No UI rendered |
| `RemindersReadyNotice` | `components/transaction/RemindersReadyNotice.tsx` | Out of scope | Returns null. Polls reminder count and fires toast on `?newFile=1`. No UI rendered |
| `ChainClaimedNotice` | `components/transaction/ChainClaimedNotice.tsx` | Out of scope | Returns null. Fires toast on `?claimed=1`. No UI rendered |
| `TransactionNotes` | `components/transaction/TransactionNotes.tsx` | Match polish page | Overview tab: note list + add-note textarea. Uses `useOptimistic` |
| `BrokerSection` | `components/transaction/BrokerSection.tsx` | Out of scope | Overview tab: broker referral fee + toggle. Returns null when `!brokerFirmId`. Not in polish page |
| `ContactsSection` | `components/contacts/ContactsSection.tsx` | No changes — already matches polish | Overview tab: vendor/purchaser contacts; inline add/edit/delete; portal invite/link actions |
| `SolicitorSection` | `components/solicitors/SolicitorSection.tsx` | No changes — already matches polish | Overview tab: solicitor picker per side, referral fee, intel badge |
| `PropertyIntelCard` | `components/property/PropertyIntelCard.tsx` | Match polish page | Overview tab: Land Registry price history + EPC data. Three states: loading / error / data |
| `MilestonePanel` | `components/milestones/MilestonePanel.tsx` | Match polish page | Milestones tab: side tabs, section accordions, progress bar/card. Contains 5 bespoke keyframes |
| `MilestoneRow` | `components/milestones/MilestoneRow.tsx` | Match polish page | Single milestone row; Confirm / Undo / N/R actions. Uses `useOptimistic` |
| `NotRequiredRow` | `components/milestones/NotRequiredRow.tsx` | Match polish page | N/R section row; Reinstate action. Opens `MortgageModal` for PM9 |
| `SurveyNrConfirmModal` | `components/milestones/SurveyNrConfirmModal.tsx` | Match polish page | PM9 N/R confirmation; portal modal |
| `UndoMilestoneModal` | `components/milestones/UndoMilestoneModal.tsx` | Match polish page | Two-option undo picker (target only vs cascade); portal modal |
| `MortgageModal` | `components/milestones/MortgageModal.tsx` | Match polish page | PM9 reinstate: asks if buyer now has mortgage; portal modal |
| `ReconciliationDrawer` | `components/milestones/ReconciliationDrawer.tsx` | Match polish page | Exchange/completion reconciliation; portal drawer |
| `ExchangeCelebration` | `components/milestones/ExchangeCelebration.tsx` | Match polish page | Exchange confirmed overlay; confetti canvas + modal card. Bespoke `exchange-in` keyframe |
| `RemindersSection` | `components/reminders/RemindersSection.tsx` | Match polish page | Reminders tab: urgency groups (escalated/overdue/due_today/upcoming/snoozed/completed), per-row snooze, Chase CTA columns. Opens `ChaseDrawer` |
| `ChaseDrawer` | `components/chase/ChaseDrawer.tsx` | Match polish page | Portal right-hand drawer: channel selector, tone picker, AI generate, send. Opened by RemindersSection and ViewChainButton's ChainDrawer context |
| `ManualTaskList` | `components/todos/ManualTaskList.tsx` | Match polish page | To-Do tab: my tasks + agent requests sections. Optimistic add. Opens `AddManualTaskForm` and `ManualTaskCard` |
| `ActivityTimeline` | `components/activity/ActivityTimeline.tsx` | Match polish page | Activity tab: filterable/searchable timeline of milestones and comms. Delete comms. Show more/less |
| `CommsEntry` | `components/activity/CommsEntry.tsx` | Match polish page | Activity tab: 4-step wizard to log a communication or internal note |
| `EmailParseWidget` | `components/activity/EmailParseWidget.tsx` | Out of scope | Activity tab: "coming soon" smart email reader banner. Dismissible. Not in polish visual spec |
| `ComposeEmail` | `components/verified-emails/ComposeEmail.tsx` | Match polish page | Activity tab: compose and send email via verified sender. Three states: no-sender / compose / sent |
| `ViewChainButton` | `components/chain/ViewChainButton.tsx` | Match polish page | Overview tab: "View Chain" button; opens `ChainDrawer` (chain view) and `AddNodeDrawer` |
| `ChainDrawer` | `components/chain/ChainDrawer.tsx` | Match polish page | Portal right-hand drawer: loads chain via API; states: loading / no chain / empty / populated. Opens `AddNodeDrawer` |
| `AddNodeDrawer` | `components/chain/AddNodeDrawer.tsx` | Match polish page | Portal drawer for adding/editing chain link stubs |
| `TransactionViewTracker` | `components/agent/TransactionViewTracker.tsx` | Out of scope | Returns null. Fires PostHog analytics + `trackView` on mount. No UI rendered |
...
|---|---|---|---|---|
| **Loading skeleton** | Topbar shimmer 56px, hero shimmer 96px, three `agent-skeleton` blocks (~160/200/140px). No visible text. | `app/agent/transactions/[id]/loading.tsx` | Matches — documented in §4. | Verify only — no changes expected. |
| **FileHealthBanner** | `agent-reveal-in rounded-xl border px-4 py-3 flex items-center justify-between`. No icon. Amber: `border-amber-200 bg-amber-50/80`. Red: `border-red-200 bg-red-50/80`. Text left, "View reminders →" `agent-link` floated right. | `components/transaction/FileHealthBanner.tsx` | Production uses `flex items-start gap-3` with an `<svg>` alert-triangle icon preceding the text. Text and link are stacked left, not side-by-side. | Remove `<svg>` icon; change `items-start gap-3` → `items-center justify-between`; ensure link is right-aligned. |
| **PropertyHero (agent variant)** | Warm gradient card. Address `text-lg font-bold`. Status `agent-pill agent-pill-*`. "← Back" `agent-link-muted`. Horizontal progress bar. | `components/transaction/PropertyHero.tsx` | Production is richer than polish (radial bloom gradients, agency name display, "My Files" breadcrumb). Back link, status pill, and progress bar are all present and structurally correct. Polish is a simplified mockup. | Verify "← Back" uses `agent-link-muted`; verify status badge uses `agent-pill agent-pill-active/hold/withdrawn/completed`. No structural removals — production enrichments are acceptable. |
| **Tab bar (PropertyFileTabs)** | Five tabs using `agent-tab` class. Active tab has coral underline. Glass background. | `components/transaction/PropertyFileTabs.tsx` | Production renders as `sticky top-0 z-20 glass-nav` with `agent-tab-bar` + `agent-tab` on each button. Coral underline present. Structural position differs (separate sticky strip vs. embedded in hero in polish) — acceptable production decision. | Verify all five tab buttons have `agent-tab` class. No other changes. |
| **Overview — NextMilestoneWidget** | `glass-card overflow-hidden rounded-[12px]`. CardHdr: `px-4 py-3 border-b border-white/20`, title `text-xs font-semibold text-slate-900/70` "Next steps". Body: `divide-y divide-white/15`, rows `px-4 py-3`. | `components/transaction/NextMilestoneWidget.tsx` | `glass-card` wrapper present; CardHdr `px-4 py-3 border-b border-white/20` ✓; title `text-xs font-semibold text-slate-900/70` ✓. Gap: `glass-card` outer wrapper may be missing `overflow-hidden rounded-[12px]`. Row padding `px-4 py-3` — verify. | Add `overflow-hidden rounded-[12px]` to outer `glass-card` wrapper if absent; confirm row padding is `px-4 py-3`. |
| **Overview — RemindersWidget** | `glass-card overflow-hidden rounded-[12px]`. CardHdr: title "Reminders" + count `agent-badge`. Body rows `px-4 py-3`. "View all →" `agent-link` in header right. | `components/transaction/RemindersWidget.tsx` | `glass-card` wrapper ✓; CardHdr ✓; `agent-badge` count pill ✓; `agent-link` "View all →" ✓. Gap: `overflow-hidden rounded-[12px]` may be missing from wrapper. | Add `overflow-hidden rounded-[12px]` to outer `glass-card` wrapper if absent. |
| **Overview — RecentActivityWidget** | `glass-card overflow-hidden rounded-[12px]`. CardHdr: title "Recent activity" + "View all →" `agent-link` right. Body rows `px-4 py-3`. Comm type label: coral text. | `components/activity/RecentActivityWidget.tsx` | `glass-card` ✓; CardHdr ✓; `agent-link` ✓. Gap: `overflow-hidden rounded-[12px]` may be absent; production comm type label uses `text-slate-900/40` (muted) vs. polish coral. | Add `overflow-hidden rounded-[12px]` to wrapper; change comm type label colour from muted (`text-slate-900/40`) to coral (`text-[var(--agent-coral-deep)]`). |
| **Overview — RiskScoreWidget** | `glass-card overflow-hidden rounded-[12px]`. CardHdr: `px-4 py-3 border-b border-white/20`, title `text-xs font-semibold text-slate-900/70` "Fall-through risk". Risk level badge in header right. Body `px-4 py-4`. | `components/transaction/RiskScoreWidget.tsx` | Header uses `px-5 py-4 border-b border-white/20 flex items-center justify-between`. Title `text-sm font-semibold text-slate-900/90` — wrong padding, size, and colour. Body `px-5 py-4`. Risk badge placed inline in header (position is fine). | Change header padding `px-5 py-4` → `px-4 py-3`; change title class `text-sm font-semibold text-slate-900/90` → `text-xs font-semibold text-slate-900/70`; add `overflow-hidden rounded-[12px]` to outer wrapper. |
| **Overview — ContactsSection** | `glass-card overflow-hidden rounded-[12px]`. CardHdr: title "Contacts" + count badge + "+ Add contact" link. Body `divide-y divide-white/20` contact cards `px-5 py-4`. | `components/contacts/ContactsSection.tsx` | Updated in Commit 3. Verified from file read: structure matches polish. | Verify only — no changes expected. |
| **Overview — SolicitorSection** | `glass-card overflow-hidden rounded-[12px]`. CardHdr: title "Solicitors". Body `divide-y divide-white/20`, vendor card + purchaser card `px-5 py-4`. | `components/solicitors/SolicitorSection.tsx` | Updated in Commit 3. Verified from file read: `glass-card overflow-hidden rounded-[12px]` ✓; CardHdr `px-4 py-3 border-b border-white/20` ✓; title `text-xs font-semibold text-slate-900/70` ✓; body padding and divide ✓. | Verify only — no changes expected. |
| **Overview — PropertyIntelCard** | `glass-card overflow-hidden rounded-[12px]`. CardHdr: title `text-xs font-semibold text-slate-900/70` "Property Intel"; external links (Rightmove / Zoopla / Land Reg) as `agent-link` buttons in header right. Body `px-5 py-4` with stacked data sections. | `components/property/PropertyIntelCard.tsx` | Header: `px-5 py-4 border-b border-white/20 flex items-center justify-between`. Title `text-sm font-semibold text-slate-900/90` — wrong. External links in header right ✓. Body `px-5 py-4` ✓. Data layout: `flex gap-6` side-by-side vs. polish stacked. | Change header padding `px-5 py-4` → `px-4 py-3`; change title `text-sm font-semibold text-slate-900/90` → `text-xs font-semibold text-slate-900/70`; add `overflow-hidden rounded-[12px]` to outer wrapper; change data layout from `flex gap-6` to stacked (`space-y-3` or similar). |
| **Overview — TransactionNotes** | `glass-card overflow-hidden rounded-[12px]`. CardHdr: title "Notes". Body `px-5 py-4`: textarea above note list. Note rows `px-4 py-3 group`. Delete: `agent-link-muted opacity-0 group-hover:opacity-100`. | `components/transaction/TransactionNotes.tsx` | Uses `<section>` wrapper + `<h2 className="uppercase tracking-wide …">` heading — no CardHdr. Form uses `glass-card clipPath` styling. Note rows: `group glass-card px-4 py-3.5` (individual cards, not divide-y). | Replace `<section>` + `<h2>` with `glass-card overflow-hidden rounded-[12px]` outer wrapper + CardHdr "Notes"; move textarea + note list inside `px-5 py-4` body with `divide-y divide-white/15`; change note rows from individual `glass-card` to plain `px-4 py-3 group` rows. |
| **Overview — ViewChainButton** | `agent-link` "View chain" text button. Conditionally rendered — only shown when a chain exists. Opens `ChainDrawer`. | `components/chain/ViewChainButton.tsx` | Button uses `agent-link-primary` (non-canonical). `font-medium transition-colors` also present but redundant once canonical class applied. | Change `agent-link-primary` → `agent-link` on the "View Chain" button. |
| **Sidebar — Progress card** | `GlassCard` with `padding: 16px`. `.agent-sidebar-label` "PROGRESS" `marginBottom: 12px`. Below: flex row `gap: 16px`. Left: SVG ring **72×72** (r=28, strokeWidth=6), stroke always `var(--agent-coral-deep)`. Centered inside ring: single "N%" string at fontSize 15, fontWeight 700, fill TP. Right col: `agent-pill agent-pill-active` (fontSize 10) stacked above `fontSize: 11, color: TM` "N weeks elapsed". | `components/transaction/TransactionSidebar.tsx` (`ProgressRing` component + Progress section, lines 26–57, 141–156) | Ring is **80×80** (r=32, strokeWidth=8). Stroke is dynamic: green/amber/red/blue by `onTrack` prop — not always coral. Percentage displayed as two stacked spans: `text-lg font-bold text-slate-900/80` number + `text-[10px] text-slate-900/40 font-medium` "%" — not a single string. Outer padding: `p-5` (20px) vs polish 16px. | In `ProgressRing`: `width/height 80→72`, `r 32→28`, `strokeWidth 8→6`; remove dynamic stroke color — set `stroke` to `var(--agent-coral-deep)` constant; collapse two stacked spans into one combined "N%" span. On Progress `glass-card` wrapper: `p-5` → `p-4`. |
| **Sidebar — Exchange Forecast card** | `GlassCard` with `padding: "12px 16px"`. `.agent-sidebar-label` "EXCHANGE FORECAST". Each date row: **horizontal table** layout — `display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8`. Label LEFT: fontSize 11, color TM (muted). Value RIGHT: fontSize 11, fontWeight 600, color TP. No stacking. | `components/transaction/TransactionSidebar.tsx` (Exchange Forecast section, lines 158–231) | **Stacked layout** — `space-y-3` blocks; label (`text-xs text-slate-900/40 mb-0.5`) sits above value (`text-sm font-semibold text-slate-900/90`) in each block. Padding `p-5` (20px). Date values at `text-sm` (14px) vs polish fontSize 11. Note: the stacked/table split is the reverse of what the previous row description stated — production is stacked, polish is horizontal table. | Restructure each date block from stacked (`<div>label<br/>value</div>`) to `flex justify-between items-center`; move label LEFT, value RIGHT on same row; change date value from `text-sm font-semibold` → `text-xs font-semibold` throughout; change outer padding `p-5` → `px-4 py-3`. |
| **Sidebar — Agent card** | Not present in polish mock sidebar. | `components/transaction/TransactionSidebar.tsx` (Agent section, lines 233–241) | Present in production: `glass-card p-5`. Not in polish page. | Out of scope — production-only feature. No changes. |
| **Sidebar — Price & Fees card** | `glass-card px-4 py-3`. Header `flex items-center justify-between`: `.agent-sidebar-label` "Price & Fees" left + "Edit details" `agent-link` right. Fee rows `pt-2 border-t border-white/20` with `text-xs` labels + `text-sm font-semibold` values. Net income row: `pt-3 mt-1 border-t-2 border-white/40`. | `components/transaction/TransactionSidebar.tsx` (Price & Fees section, lines 243–355) | `glass-card p-5`. Header `flex items-center justify-between mb-4` ✓. `agent-link` "Edit details" ✓. Fee row structure matches. Gap: outer padding `p-5` vs. `px-4 py-3`; `mb-4` after header (should become `mb-3` at smaller padding). | Change `p-5` → `px-4 py-3` on Price & Fees card wrapper; adjust header `mb-4` → `mb-3`. |
| **Milestones tab — MilestonePanel (outer + side tabs + accordions)** | `glass-card overflow-hidden rounded-[12px]` outer. Progress card at top. Vendor/Purchaser tabs: `agent-segment-pill` for active state. Section accordion headers: `agent-acc-hdr`. Accordion body: `agent-acc`/`agent-acc-in`. | `components/milestones/MilestonePanel.tsx` | Progress card ✓ (bespoke keyframes documented in §10). Side tabs: active state uses `bg-white/60 shadow-sm` — not `agent-segment-pill`. Section accordions: `transition-all group` Tailwind — not `agent-acc-hdr`. Outer wrapper: verify `glass-card overflow-hidden rounded-[12px]`. | Wire `agent-segment-pill` on Vendor/Purchaser side tab active state; wire `agent-acc-hdr` on section accordion header buttons; wire `agent-acc`/`agent-acc-in` on accordion content divs; verify outer wrapper has `overflow-hidden rounded-[12px]`. |
| **Milestones tab — MilestoneRow** | Row: milestone dot + name + completion meta + action button. Confirm/Undo/NR buttons: `agent-btn`. Inline forms (event date, N/R reason, counterpart notice): enter with `agent-reveal-in`, exit with `agent-reveal-out`. | `components/milestones/MilestoneRow.tsx` | Confirm/Undo/Cancel/NR/OK buttons: bespoke Tailwind — not `agent-btn`. Inline forms: conditional render without transition class. | Add `agent-btn` to Confirm, Undo, NR, Cancel, OK buttons (observer: do not touch `useOptimistic` logic, only the className prop); wire `agent-reveal-in`/`agent-reveal-out` on event date form, N/R reason form, and counterpart notice divs. |
| **Milestones tab — NotRequiredRow** | Muted row. "Reinstate" action: `agent-link`. | `components/milestones/NotRequiredRow.tsx` | "Reinstate" button: bespoke Tailwind hover — not `agent-link`. | Change "Reinstate" button className to `agent-link`. |
| **Milestones tab — Exchange Ready Banner** | Emerald/green reveal-in banner replacing the progress-bar card when both sides are exchange-ready. `agent-reveal-in` entrance. `var(--agent-success-bg)` background, `var(--agent-success-border)` border, `borderRadius: 10`. Flex `items-center gap-3`: circular green icon dot + right col with "Ready to exchange" headline (fontWeight 600) + muted subtitle. | `components/milestones/MilestonePanel.tsx` (`exchangeReady` branch, lines 233–244) | `mb-5 px-4 py-3 rounded-xl bg-emerald-500/15 border border-emerald-400/25 flex items-center gap-3 animate-enter`. `animate-enter` is non-canonical. `bg-emerald-500/15 border border-emerald-400/25` are hardcoded Tailwind colors, not CSS vars. Icon dot: `bg-green-500` (hardcoded). | Replace `animate-enter` → `agent-reveal-in`; replace `bg-emerald-500/15` → `var(--agent-success-bg)`; replace `border-emerald-400/25` → `var(--agent-success-border)`. |
| **Reminders tab — RemindersSection** | `glass-card overflow-hidden rounded-[12px]`. CardHdr: "Reminders & Tasks". Two-column body (Seller/Buyer). Group headers (Escalated/Overdue/etc.): `agent-acc-hdr`. ✓ Done row: fires `agent-row-flash` on complete. | `components/reminders/RemindersSection.tsx` | Updated Commit 4: `glass-card overflow-hidden rounded-[12px]` ✓; CardHdr ✓. Gap: urgency group header buttons use bespoke Tailwind — not `agent-acc-hdr`. ✓ Done completion: no `agent-row-flash`. | Wire `agent-acc-hdr` on each urgency group header button (Escalated/Overdue/Due today/Coming up/Snoozed/Completed); wire `agent-row-flash` on the row container when ✓ Done is triggered. |
| **To-Do tab — ManualTaskList** | Two separate `glass-card overflow-hidden rounded-[12px]` blocks stacked in `space-y-4`. (1) CardHdr "To-Do" + `agent-badge` count; task rows `px-4 py-2.5`; delete buttons `agent-icon-btn agent-icon-btn-sm`; done-section expand `agent-acc`/`agent-acc-in`; "Show N done" toggle `agent-link-muted`. (2) CardHdr "With Sales Progressor" + `agent-badge` count; request rows as `divide-y` rows. | `components/todos/ManualTaskList.tsx` | Outer wrapper is `<div className="space-y-6">` — no glass-card, no CardHdr on either section. "My to-dos" header is `glass-subtle px-4 py-3 rounded-xl` bar (not CardHdr). `agent-acc`/`agent-acc-in` already wired on done-section ✓ (keep). "With Sales Progressor" uses `<h2 className="text-sm font-semibold text-slate-900/80">` + bespoke `bg-amber-100/80 text-amber-700` badge (not CardHdr + `agent-badge`). `AgentRequestRow`: individual `glass-card px-4 py-3.5` per row (not divide-y). | Wrap each section in its own `glass-card overflow-hidden rounded-[12px]` + CardHdr; replace `glass-subtle` "My to-dos" bar with CardHdr "To-Do" + `agent-badge`; replace `<h2>` + bespoke badge with CardHdr "With Sales Progressor" + `agent-badge`; change `AgentRequestRow` from individual `glass-card` wrapper to plain `px-4 py-3 divide-y` row. `agent-acc`/`agent-acc-in` already wired — no change needed there. |
| **Activity tab — ActivityTimeline** | Filter bar: All/Milestones/Comms/Automated/Notes buttons use `agent-segment-pill`. Search input: `agent-focus`. Timeline entries: `divide-y divide-white/10` rows. | `components/activity/ActivityTimeline.tsx` | Filter buttons: active state uses `bg-slate-900/15` — not `agent-segment-pill`. Search input: `focus:ring-1 focus:ring-blue-300/50` — not `agent-focus`. Timeline entry divide: verify `divide-y` present. | Wire `agent-segment-pill` on filter buttons (All/Milestones/Comms/Automated/Notes); change search input focus style from `focus:ring-1 focus:ring-blue-300/50` to `agent-focus`. |
| **Activity tab — CommsEntry** | Collapsed: dashed-border prompt. Step transitions use `agent-reveal-in`/`agent-reveal-out`. Step buttons: `agent-btn`. | `components/activity/CommsEntry.tsx` | Step transitions: conditional render without animation. Type/method buttons: bespoke Tailwind — not `agent-btn`. | Wire `agent-reveal-in`/`agent-reveal-out` on each step content div; add `agent-btn` to type selection buttons (Internal note/Outbound/Inbound) and method buttons. |
| **Activity tab — ComposeEmail** | Three distinct states. (1) **No-sender banner**: `agent-reveal-in`, `var(--agent-warning-bg)` bg, `0.5px solid var(--agent-warning-border)`, `borderRadius: 10`, `items-center justify-between`. Settings link `agent-link`. Dismiss × `agent-icon-btn agent-icon-btn-sm`. (2) **Compose card**: `padding: 16px`, `rgba(255,255,255,0.6)` bg, `borderRadius: 12`, `0.5px solid BORDER`. Labels `agent-label`. Inputs `agent-input agent-input-sm`. Send `agent-btn agent-btn-sm agent-btn-primary`. Cancel `agent-btn agent-btn-sm agent-btn-ghost-bordered`. (3) **Sent banner**: `agent-reveal-in`, `var(--agent-success-bg)`, `0.5px solid var(--agent-success-border)`, `borderRadius: 10`. | `components/verified-emails/ComposeEmail.tsx` | (1) No-sender: `flex items-start gap-3 px-4 py-4 rounded-xl bg-amber-50 border border-amber-100`; no `agent-reveal-in`; settings link `underline font-medium` (not `agent-link`); dismiss × `rounded-full hover:bg-amber-100` (not `agent-icon-btn`); `items-start` (not `items-center justify-between`). (2) Compose: `glass-card p-5 space-y-3`; labels `text-[11px] font-semibold text-slate-900/40 uppercase tracking-wide` (not `agent-label`); inputs `glass-input` (not `agent-input agent-input-sm`); Send `agent-btn-color-primary disabled:opacity-40` (missing `agent-btn agent-btn-sm` bases); Cancel bespoke hover (not `agent-btn-ghost-bordered`). (3) Sent: `px-4 py-4 rounded-xl bg-emerald-50 border border-emerald-100`; no `agent-reveal-in`; hardcoded colors. | (1) No-sender: add `agent-reveal-in`; `bg-amber-50` → `var(--agent-warning-bg)`; `border-amber-100` → `var(--agent-warning-border)`; `items-start` → `items-center justify-between`; settings link → `agent-link`; dismiss × → `agent-icon-btn agent-icon-btn-sm`. (2) Compose: `glass-input` → `agent-input agent-input-sm`; labels → `agent-label`; Send → `agent-btn agent-btn-sm agent-btn-primary`; Cancel → `agent-btn agent-btn-sm agent-btn-ghost-bordered`. (3) Sent: add `agent-reveal-in`; `bg-emerald-50` → `var(--agent-success-bg)`; `border-emerald-100` → `var(--agent-success-border)`. |
| **ChaseDrawer** | Right-hand drawer (`agent-drawer-in` ✓). Close ×: `agent-icon-btn`. Tone dropdown: `agent-dropdown-in`. Generate/Send buttons: canonical. | `components/chase/ChaseDrawer.tsx` | `agent-drawer-in` ✓. Close ×: inline hover handler — not `agent-icon-btn`. Tone dropdown: appears instantly — not `agent-dropdown-in`. Channel crossfade: 120ms inline opacity (non-canonical; reduced-motion fix pending from §10). | Wire `agent-icon-btn` on close × button; wire `agent-dropdown-in` on tone picker dropdown. |
| **EditSaleDetailsDrawer** | Right-hand drawer (`agent-drawer-in` ✓). Close ×: `agent-icon-btn`. PropSaveStage conditional sections: `agent-reveal-in`/`agent-reveal-out`. Cancel buttons: `agent-btn-ghost-bordered`. | `components/transaction/EditSaleDetailsDrawer.tsx` | `agent-drawer-in` ✓. Close ×: inline hover — not `agent-icon-btn`. PropSaveStage sections: conditional render without transition. Cancel buttons: bespoke Tailwind. | Wire `agent-icon-btn` on close ×; wire `agent-reveal-in`/`agent-reveal-out` on PropSaveStage conditional section divs; wire `agent-btn-ghost-bordered` on Cancel buttons throughout. |
| **AddNodeDrawer** | Right-hand drawer (`agent-drawer-in` ✓). Close ×: `agent-icon-btn`. Cancel: `agent-btn-ghost-bordered`. All inputs: `agent-focus`. | `components/chain/AddNodeDrawer.tsx` | `agent-drawer-in` ✓. Close ×: inline hover — not `agent-icon-btn`. Cancel: bespoke Tailwind. Email field: `focus:ring-2 focus:ring-blue-300/50` — not `agent-focus` (other fields use `agent-focus` ✓). | Wire `agent-icon-btn` on close ×; wire `agent-btn-ghost-bordered` on Cancel; change email input focus from `focus:ring-2 focus:ring-blue-300/50` → `agent-focus`. |
| **ChainDrawer** | Right-hand drawer (inline `agent-drawer-in` animation ✓). Close ×: `agent-icon-btn`. Delete cancel: `agent-btn-ghost-bordered`. Add-above/below dashed buttons: `agent-link`. | `components/chain/ChainDrawer.tsx` | Animation ✓ (inline). Close ×: inline hover — not `agent-icon-btn`. Delete cancel: bespoke. Add-above/below: uses `agent-hover-link` (non-canonical — investigate). | Wire `agent-icon-btn` on close ×; wire `agent-btn-ghost-bordered` on delete cancel; investigate `agent-hover-link` — canonicalise to `agent-link` or remove as documented in §10.5. |
| **Modals — SurveyNrConfirmModal, UndoMilestoneModal, MortgageModal** | Portal modal (`agent-modal-in` ✓). Close ×: `agent-icon-btn`. Primary action: canonical primary button. Cancel: `agent-btn-ghost-bordered`. | `components/milestones/SurveyNrConfirmModal.tsx`, `UndoMilestoneModal.tsx`, `MortgageModal.tsx` | `agent-modal-in` ✓ on all three. Close ×: inline hover — not `agent-icon-btn` on all three. Cancel buttons: bespoke Tailwind. | Wire `agent-icon-btn` on close × in all three modals; wire `agent-btn-ghost-bordered` on Cancel in all three modals. |
| **ExchangeCelebration** | Full-screen overlay. Confetti canvas + modal card (`exchange-in` keyframe). "Continue": canonical primary button. Reduced-motion guard needed on both canvas and keyframe. | `components/milestones/ExchangeCelebration.tsx` | `exchange-in` keyframe present; confetti canvas `requestAnimationFrame` loop. No reduced-motion guard on either. "Continue" button: verify class. | Add `@media (prefers-reduced-motion: reduce)` guard to `exchange-in` keyframe; add `prefers-reduced-motion` check to skip canvas loop. Verify "Continue" uses canonical primary button class. (Reduced-motion items flagged as Stage 2 in §10 — confirm done before Stage 4 sign-off.) |
| **StatusControl dropdown** | Status dropdown: `agent-dropdown-in` on open. Withdrawal reason form: `agent-reveal-in`. | `components/transaction/StatusControl.tsx` | Dropdown: appears instantly — not `agent-dropdown-in`. Withdrawal reason form: conditional render without transition. | Wire `agent-dropdown-in` on status dropdown; wire `agent-reveal-in`/`agent-reveal-out` on withdrawal reason textarea. |
...
| 2026-05-11 | Progress ring verified: 72×72/r28/sw6/always-coral in polish vs 80×80/r32/sw8/dynamic-color in production; percentage display differs | Section 13 — Sidebar: Progress card |
| 2026-05-11 | Exchange Forecast layout correction: polish is horizontal-table (label-left, value-right), production is stacked (label-above, value-below) — direction of change is reverse of prior row description | Section 13 — Sidebar: Exchange Forecast card |
| 2026-05-11 | ViewChainButton: agent-link-primary (non-canonical) confirmed; ManualTaskList: no outer glass-card; ComposeEmail: three-state gaps all documented; Exchange Ready Banner: animate-enter (non-canonical) confirmed, banner is emerald not amber | Section 13 — four rows updated |
| 2026-05-11 | ContactsSection fully read and confirmed matching polish; SolicitorSection confirmed from prior read. Both "No changes — already matches polish" tags stand | Section 13 — Concern 3 closed |
