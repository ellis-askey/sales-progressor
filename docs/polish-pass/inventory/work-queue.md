...
| ↳ `FilterChip` | (inline in AgentRemindersList.tsx:57) | Match polish page | Bespoke Tailwind component. Convert to `agent-segment-pill`. |
| ↳ `SideSnoozeMenu` | (inline in AgentRemindersList.tsx:86) | Match polish page | "Snooze all" per group. Already has `agent-btn agent-btn-sm agent-btn-ghost` on trigger; dropdown items use `hover:bg-slate-50` Tailwind — convert to `agent-dropdown-item`. |
| ↳ `RowSnoozeMenu` | (inline in AgentRemindersList.tsx:143) | Match polish page | Per-row snooze. Trigger uses inline styles (not `agent-btn`). Dropdown container has `agent-dropdown-in` ✓. Items use `hover:bg-slate-50` — convert to `agent-dropdown-item`. |
| ↳ `SplitFileCard` | (inline in AgentRemindersList.tsx:409) | Match polish page | File card container. `glass-card` → `agent-glass-strong`. Address header inline styles → canonical. Follows RemindersSection.ColumnSection pattern from transaction-detail (see §13 note). |
| ↳ `SideColumn` | (inline in AgentRemindersList.tsx:199) | Match polish page | Seller/Buyer column inside SplitFileCard. Chase button inline-styled (solid fill) → `agent-btn agent-btn-sm agent-btn-primary`. `agent-row-exit` already wired ✓. Follows `RemindersSection.ColumnSection` (`components/reminders/RemindersSection.tsx:162`) pattern. |
| ↳ `EmptyColumn` | (inline in AgentRemindersList.tsx:375) | No changes | Colour-coded placeholder shown when one side has no reminders. Agent token-adjacent already. |
...
| **FileAlertsStrip** | `agent-glass-strong` card, `overflow: hidden`. Header: `agent-card-hdr` (amber background tint via inline override). Header right: `agent-link-muted` Show/Hide toggle with `agent-acc` / `agent-acc-in` body. Alert rows: `agent-hover-row`. Alert action links: `agent-link-muted`. Badge chips: keep existing amber/red/sky token colours. **NEW PATTERN** — no transaction-detail equivalent. Justification: FileAlertsStrip is a work-queue-specific feature surfacing file-level data quality alerts (not reminders). Its collapsed-by-default design is intentional: it is secondary context, not the primary action surface. | `FileAlertsStrip` | `glass-card overflow-hidden` (not `agent-glass-strong`). Header: `flex items-center justify-between px-4 py-2.5` inline Tailwind (not `agent-card-hdr`). Toggle: Tailwind hover. Rows: `hover:bg-white/20 transition-colors`. Action links: complex Tailwind class string. Body: no `agent-acc`. | Convert `glass-card` → `agent-glass-strong`. Header → `agent-card-hdr` with amber tint override. Toggle → `agent-link-muted`. Body → `agent-acc` / `agent-acc-in`. Rows → `agent-hover-row`. Action links → `agent-link-muted`. |
