...
- **Inline animated claim-confirm (chain card → confirm, no page load).** Today the claim flow is one route per step: the `/claim` landing (chain card) sends the CTA to a separate `/claim/confirm` page for the tenure + purchase-method step (screenshot mock captured 2026-09-06). For a visitor who is **already logged in**, that confirm step doesn't need a new page — the chain card could animate out while the confirm card animates in on the same page, then post to the existing `/api/claim` (which already re-runs every guard, so there's no safety regression). Login/signup stay as their own pages — establishing a NextAuth session is a genuine separate concern. **Trigger:** the inline transition only helps visitors who are already signed in, and today ~100% of claimers are fresh signups (no account), so they route through `/claim/signup` and never reach the logged-in confirm path — building it now would be invisible. Revisit once a meaningful share of claimers already hold a TSP account (e.g. multi-file agencies claiming repeat chain invites while logged in). **Mock:** to be pinned here once Ellis's edits are named.
