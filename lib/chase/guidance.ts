...
export const TONE_GUIDANCE: Record<string, string> = {
  ...
  urgent: `Urgent tone. No emoji whatsoever, not even one. No exclamation marks. Use this when exchange or another hard deadline is genuinely at risk. Open by surfacing the SHARED goal ("we're working towards exchange, so I'm just trying to tie up the last few bits this week"). Then explain factually what's outstanding. Then ask plainly for the action. Then volunteer to do your part: "once X is in I can push everything through with the solicitor." Tone stays warm. Urgency comes from the shared stake, not pressure on the recipient. Never state a specific target exchange or completion date.`,
  final_reminder: `Final-reminder tone. Use this when multiple chases over a sustained period have gone unanswered and the transaction is at material risk. Name the timeline of attempted contact factually and without accusation ("I've sent messages on the 14th, 21st and 28th"). State the consequence plainly and as a SHARED outcome ("if I don't hear back this week, I'll need to update the chain that our planned exchange is at risk"). Still no blame. The message is "I want to avoid this together." Never state a specific target exchange or completion date.`,
};
