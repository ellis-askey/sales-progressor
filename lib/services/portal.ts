...
async function getPortalDataInner(token: string) {
  ...
  const contact = await prisma.contact.findFirst({
    ...
    select: {
      ...
      // First-visit welcome sheet gate — null until the client dismisses it.
      welcomeSeenAt: true,
  ...
  return {
    ...
    transaction: {
      ...
      clientDepositGBP: tx.clientDepositGBP,
      clientMortgageGBP: tx.clientMortgageGBP,
      clientFirstTimeBuyer: tx.clientFirstTimeBuyer,
      clientAdditionalProperty: tx.clientAdditionalProperty,
      clientCompletionFundsSent: tx.clientCompletionFundsSent,
      clientOtherFundsSentGBP: tx.clientOtherFundsSentGBP,
      tenure: tx.tenure,
...
<p><a href="${dashUrl}" style="display:inline-block;background:#3B82F6;color:#fff;padding:12px 28px;border-radius:12px;text-decoration:none;font-weight:700;font-size:14px">View file</a></p>
  ...
  if (richCopy) {
    ...
    if (tx.agentUser?.email && agentCopy && wantsEmail(tx.agentUser.id)) {
      ...
      sendEmail({ to: tx.agentUser.email, subject, html, text, from: agencyEmailFrom, replyTo }).catch(() => {});
    }
  } else {
...
function richMilestoneEmailHtml({
  ...
  const signatureBlock = isProgressor
    ...
    : serviceType === "self_managed"
      ? (canReply ? `<p style="margin:0;font-size:13px;color:#4a5162">Questions? Just reply to this email.</p>` : "")
      : whatsappNumber
...
async function sendRichMilestoneEmails(
  ...
  if (tx.agentUser?.email && vendorAgentCopy && !skipAgentEmail) {
    ...
    sendEmail({ to: tx.agentUser.email, subject, text, html, from: agencyEmailFrom, replyTo }).catch(() => {});
  }
  ...
  if (tx.assignedUser?.email && progressorCopy && !skipProgressorEmail) {
    ...
    sendEmail({ to: tx.assignedUser.email, subject, text, html, from: agencyEmailFrom, replyTo }).catch(() => {});
  }
