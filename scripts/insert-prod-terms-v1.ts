// One-shot: insert the 2026-05-payments-v1 TermsVersion row on PRODUCTION.
//
// Mirror of insert-staging-terms-v1.ts but with the PROD project ID as the
// REQUIRED target (staging refuses). Same canonical Ellis-supplied content
// in the same structured shape — single source of truth for v1 across
// environments.
//
// Run ONLY after:
//   1. The schema migration 20260525090000_terms_version_body_sections has
//      been applied to prod (via npm run db:migrate:prod yes-guard).
//   2. master has the corresponding bodySections-aware code shipped.
//
// Idempotent: no-op if a row with versionTag '2026-05-payments-v1' already
// exists. Exits 0.
...
// Same canonical content as scripts/insert-staging-terms-v1.ts.
// Single source of truth — copy any updates between the two scripts.
...
async function main() {
  ...
  if (dbProj === STAGING_PROJECT_ID) {
    ...
    console.error("================================================================");
    console.error("  This script is PRODUCTION-only. Use insert-staging-terms-v1.ts");
    console.error("  for staging.");
    console.error("================================================================");
    process.exit(1);
  }
  ...
  console.log("");
  console.log("================================================================");
  console.log("  Inserting TermsVersion row on PRODUCTION");
  console.log("================================================================");
  ...
  try {
    ...
    console.log(`✓ Inserted PROD TermsVersion row id=${row.id}`);
    console.log(`  versionTag:    ${row.versionTag}`);
    console.log(`  effectiveFrom: ${row.effectiveFrom.toISOString()}`);
    console.log("");
    console.log("Production /agent/billing should now show the disclosure for");
    console.log("any director who hasn't yet acknowledged.");
  } finally {
    await p.$disconnect();
  }
}
