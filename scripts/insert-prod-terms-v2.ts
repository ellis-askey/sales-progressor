// One-shot: insert the 2026-06-payments-v2 TermsVersion row on PRODUCTION.
//
// Mirror of insert-prod-terms-v1.ts but for v2. Same canonical Ellis-supplied
// content as the migration SQL — single source of truth for v2 across
// environments.
//
// Run ONLY after:
//   1. The migration 20260525190000_terms_version_v2 has been applied to
//      prod via npm run db:migrate:prod (which itself does the insert via
//      raw SQL). This script is the backup / re-runnable seed mechanism for
//      cases where the migration didn't run or the row was removed.
//   2. master has the corresponding v2-aware code shipped (the new
//      /billing-terms preview page exists; the disclosure component already
//      handles any TermsVersion content correctly).
//
// Idempotent: no-op if a row with versionTag '2026-06-payments-v2' already
// exists. Exits 0.
//
// Three sources to keep in sync for v2:
//   1. prisma/migrations/20260525190000_terms_version_v2/migration.sql
//   2. This script
//   3. app/billing-terms/page.tsx (public preview)
// If any one changes (and it's a material change), ship a new TermsVersion
// (v3) — don't edit v2 in place after it's been acknowledged by directors.

import { PrismaClient } from "@prisma/client";

const PROD_PROJECT_ID = "gmkfustgwipgihpmpjpr";
const STAGING_PROJECT_ID = "etidawkbqctarmsdjoxp";
const VERSION_TAG = "2026-06-payments-v2";

// Same canonical content as the v2 migration SQL.
// Single source of truth — copy any updates between the two.
...
async function main() {
  ...
  console.log("");
  console.log("================================================================");
  console.log("  Inserting TermsVersion v2 on PRODUCTION");
  console.log("================================================================");
  ...
  try {
    ...
    console.log(`✓ Inserted PROD TermsVersion v2 row id=${row.id}`);
    console.log(`  versionTag:    ${row.versionTag}`);
    console.log(`  effectiveFrom: ${row.effectiveFrom.toISOString()}`);
    console.log("");
    console.log("Production /agent/account/billing now serves v2 disclosure.");
    console.log("Directors who acknowledged v1 will re-acknowledge v2 on next card action.");
  } finally {
    await p.$disconnect();
  }
}
