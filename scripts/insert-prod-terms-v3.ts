// One-shot: insert the 2026-06-payments-v3 TermsVersion row on PRODUCTION.
//
// Mirror of insert-prod-terms-v2.ts but for v3. Same canonical Ellis-supplied
// content as the migration SQL — single source of truth for v3 across
// environments.
//
// v1 and v2 stay in the table for audit. v3 becomes the active version on
// insertion because getActiveTermsVersion() picks the latest effectiveFrom.
//
// Run ONLY after:
//   1. The migration 20260525210000_terms_version_v3 has been applied to prod
...
//   2. master has the corresponding v3-aware code shipped (the v3 preview
//      page).
//
// Idempotent: no-op if a row with versionTag '2026-06-payments-v3' already
// exists. Exits 0.
//
// Three sources to keep in sync for v3:
//   1. prisma/migrations/20260525210000_terms_version_v3/migration.sql
//   2. This script
//   3. app/billing-terms/page.tsx (public preview)
// If any one changes materially, ship v4 — don't edit v3 in place.

import { PrismaClient } from "@prisma/client";

const PROD_PROJECT_ID = "gmkfustgwipgihpmpjpr";
const STAGING_PROJECT_ID = "etidawkbqctarmsdjoxp";
const VERSION_TAG = "2026-06-payments-v3";

// Same canonical content as the v3 migration SQL.
...
async function main() {
  ...
  console.log("");
  console.log("================================================================");
  console.log("  Inserting TermsVersion v3 on PRODUCTION");
  console.log("================================================================");
  ...
  try {
    ...
    console.log(`✓ Inserted PROD TermsVersion v3 row id=${row.id}`);
    console.log(`  versionTag:    ${row.versionTag}`);
    console.log(`  effectiveFrom: ${row.effectiveFrom.toISOString()}`);
    console.log("");
    console.log("Production /agent/account/billing now serves v3 disclosure.");
    console.log("Directors who acknowledged v1 or v2 will re-acknowledge v3 on next card action.");
  } finally {
    await p.$disconnect();
  }
}
